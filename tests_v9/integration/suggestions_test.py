...
from pyatlan_v9.model.enums import AtlanConnectorType, AtlanDeleteType
from pyatlan_v9.model.group import AtlanGroup
from pyatlan_v9.model.response import AssetMutationResponse
from pyatlan_v9.model.suggestions import Suggestions
...
from tests_v9.integration.glossary_test import create_term
...
def create_glossary(client: AtlanClient, name: str) -> AtlasGlossary:
    g = AtlasGlossary.creator(name=name)
...
def owner_group(
    ...
    to_create = AtlanGroup.creator(PREFIX)
    response = client.group.creator(group=to_create)
    assert response
