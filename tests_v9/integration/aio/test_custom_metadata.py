...
from pyatlan_v9.model.group import AtlanGroup, CreateGroupResponse
from pyatlan_v9.model.structs import BadgeCondition
from pyatlan_v9.model.typedef import AttributeDef, CustomMetadataDef, EnumDef
...
async def cm_enum_update(
    ...
    r = await client.typedef.updater(enum_def)
    yield r.enum_defs[0]
...
async def create_custom_metadata_async(
    ...
    cm_def = CustomMetadataDef.creator(display_name=name, description=CM_DESCRIPTION)
    ...
    r = await client.typedef.creator(cm_def)
...
async def create_enum_async(
    ...
    enum_def = EnumDef.creator(name=name, values=values)
    r = await client.typedef.creator(enum_def)
...
async def create_glossary_async(client: AsyncAtlanClient, name: str) -> AtlasGlossary:
    ...
    to_create = AtlasGlossary.creator(name=name)
...
async def create_term_async(
    ...
    to_create = AtlasGlossaryTerm.creator(
...
async def create_group_async(
    ...
    g = AtlanGroup.creator(alias=name)
    r = await client.group.creator(g)
    return r
...
async def cm_rich_text(
    ...
    yield cm
    await wait_for_successful_custometadatadef_purge_async(CM_RICH_TEXT, client=client)


async def test_cm_rich_text_async(cm_rich_text: CustomMetadataDef):
...
async def test_remove_attribute(client: AsyncAtlanClient, cm_raci: CustomMetadataDef):
    ...
    response = await client.typedef.updater(existing)
    assert response
