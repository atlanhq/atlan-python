...
async def expired_token(
    ...
    token = None
    try:
        token = await token_client.token.creator(
...
async def argo_fake_token(
    ...
    token = None
    try:
        token = await token_client.token.creator(f"{MODULE_NAME}-fake-argo")
        yield token
    finally:
        await delete_token_async(token_client, token)
...
async def glossary(
    ...
    g = AtlasGlossary.creator(name=MODULE_NAME)
