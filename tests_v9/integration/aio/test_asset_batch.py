...
from pyatlan_v9.model.enums import AssetCreationHandling
from pyatlan_v9.model.fluent_search import FluentSearch
from pyatlan_v9.test_utils import get_random_connector
...
async def schema(
    ...
    response = await client.asset.save(schema1)
...
async def batch_table_create(
    ...
    for asset in reversed(batch.created):
        ...
        created = await client.asset.get_by_qualified_name(
        ...
        response = await client.asset.purge_by_guid(created.guid)
        if (
            not response
...
async def test_batch_update(
    wait_for_consistency, client: AsyncAtlanClient, batch_table_create: AsyncBatch
):
    ...
    response = await client.asset.purge_by_guid(created_table.guid)
