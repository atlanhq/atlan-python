...
from pyatlan_v9.model.group import AtlanGroup, CreateGroupResponse, GroupRequest
...
async def group(client: AsyncAtlanClient) -> AsyncGenerator[CreateGroupResponse, None]:
    to_create = AtlanGroup.creator(GROUP_NAME)
    fixed_user = await client.user.get_by_username(FIXED_USER)
    assert fixed_user
    g = await client.group.creator(group=to_create, user_ids=[str(fixed_user.id)])
    yield g
...
async def test_update_groups(client: AsyncAtlanClient, group: CreateGroupResponse):
    ...
    await client.group.updater(group1)
