...
from pyatlan_v9.errors import NotFoundError
from pyatlan_v9.model.oauth_client import OAuthClientCreateResponse, OAuthClientResponse
...
async def pagination_oauth_clients(
    ...
    for i in range(PAGINATION_CLIENT_COUNT):
        response = await client.oauth_client.creator(
...
async def test_oauth_client_update_description(
    ...
    updated = await client.oauth_client.updater(
...
async def test_oauth_client_create_with_invalid_role_raises_error(
    ...
    with pytest.raises(NotFoundError) as exc_info:
        await client.oauth_client.creator(
