...
from pyatlan_v9.model.query import QueryRequest
from tests_v9.integration.client import TestId, delete_asset
from tests_v9.integration.requests_test import delete_token
from tests_v9.integration.utils import find_purposes_by_name_with_retry
...
def token(token_client: AtlanClient) -> Generator[ApiToken, None, None]:
    token = None
    try:
        token = token_client.token.creator(API_TOKEN_NAME)
        assert token
        assert token.guid
...
def purpose(
    ...
    to_create = Purpose.creator(name=MODULE_NAME, atlan_tags=[atlan_tag_name])
...
def test_retrieve_purpose(
    ...
    one = client.asset.get_by_qualified_name(
        qualified_name=purpose.qualified_name,
        asset_type=Purpose,
