...
from pyatlan_v9.model.enums import AtlanIcon, AtlanTagColor, TagIconType
from pyatlan_v9.model.typedef import AtlanTagDef
from tests_v9.integration.client import TestId
from tests_v9.integration.utils import wait_for_successful_tagdef_purge
...
def make_atlan_tag(
    ...
    def _make_atlan_tag(
        ...
        atlan_tag_def = AtlanTagDef.creator(name=name, color=color, image=image)
        r = client.typedef.creator(atlan_tag_def)
...
def atlan_tag_with_image(
    ...
    cls = AtlanTagDef.creator(name=CLS_IMAGE, color=AtlanTagColor.YELLOW, image=image)
    yield client.typedef.creator(cls).atlan_tag_defs[0]
