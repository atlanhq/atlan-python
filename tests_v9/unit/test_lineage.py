...
"""
Unit tests for lineage — ported from tests/unit/test_lineage.py.

Uses v9 msgspec.Struct lineage models (LineageRelation, LineageResponse,
FluentLineage, etc.) and re-exported plain dataclass types (LineageGraph,
DirectedPair).  The ``Asset`` ClassVar field references (CERTIFICATE_STATUS,
STATUS, NAME, etc.) are imported from the legacy model because the v9 Asset
model does not expose those ClassVar search fields.
"""
...
def lineage_response(lineage_json):
    return msgspec.convert(lineage_json, LineageResponse)
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                GOOD_WHERE_ASSETS,
                GOOD_WHERE_RELATIONSHIPS,
                # v9 uses plain `int` → "value is not a valid int"
