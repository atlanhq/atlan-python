...
"""Unit tests for QuickSightFolder model in pyatlan_v9."""

import pytest

from pyatlan_v9.model import QuickSightFolder
from pyatlan_v9.model.enums import QuickSightFolderType
...
def test_overload_creator():
    """Test creator supports quick_sight_folder_type input."""
    sut = QuickSightFolder.creator(
        name=QUICK_SIGHT_NAME,
        connection_qualified_name=QUICK_SIGHT_CONNECTION_QUALIFIED_NAME,
        quick_sight_id=QUICK_SIGHT_ID,
...
def test_updater_with_invalid_parameter_raises_value_error(
    ...
    with pytest.raises(ValueError, match=message):
        QuickSightFolder.updater(qualified_name=qualified_name, name=name)


def test_updater():
    """Test updater creates a QuickSightFolder for modification."""
