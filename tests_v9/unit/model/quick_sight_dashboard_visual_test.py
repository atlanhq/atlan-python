...
"""Unit tests for QuickSightDashboardVisual model in pyatlan_v9."""

import pytest

from pyatlan_v9.models import QuickSightDashboardVisual
from tests_v9.unit.model.constants import (
    QUICK_SIGHT_CONNECTION_QUALIFIED_NAME,
    QUICK_SIGHT_CONNECTOR_TYPE,
    QUICK_SIGHT_DASHBOARD_VISUAL_QUALIFIED_NAME,
    QUICK_SIGHT_ID_DASHBOARD_VISUAL,
    QUICK_SIGHT_NAME,
...
@pytest.mark.parametrize(
    "name, quick_sight_id, quick_sight_sheet_id, quick_sight_sheet_name, quick_sight_dashboard_qualified_name, message",
    [
        (
            None,
            QUICK_SIGHT_ID_DASHBOARD_VISUAL,
        ...
        (
            ...
            QUICK_SIGHT_SHEET_NAME,
            None,
            "quick_sight_dashboard_qualified_name is required",
        ),
    ],
)
def test_creator_with_missing_parameters_raise_value_error(
    ...
    with pytest.raises(ValueError, match=message):
        QuickSightDashboardVisual.creator(
            name=name,
...
def test_creator():
    ...
    sut = QuickSightDashboardVisual.creator(
        name=QUICK_SIGHT_NAME,
        quick_sight_id=QUICK_SIGHT_ID_DASHBOARD_VISUAL,
        quick_sight_sheet_id=QUICK_SIGHT_SHEET_ID,
        quick_sight_sheet_name=QUICK_SIGHT_SHEET_NAME,
        quick_sight_dashboard_qualified_name=QUICK_SIGHT_QUALIFIED_NAME,
    )

    assert sut.name == QUICK_SIGHT_NAME
    assert sut.quick_sight_dashboard_qualified_name == QUICK_SIGHT_QUALIFIED_NAME
    assert sut.quick_sight_id == QUICK_SIGHT_ID_DASHBOARD_VISUAL
    assert sut.qualified_name == QUICK_SIGHT_DASHBOARD_VISUAL_QUALIFIED_NAME
...
def test_updater_with_invalid_parameter_raises_value_error(
    ...
    with pytest.raises(ValueError, match=message):
        QuickSightDashboardVisual.updater(qualified_name=qualified_name, name=name)


def test_updater():
    """Test updater creates a QuickSightDashboardVisual for modification."""
    sut = QuickSightDashboardVisual.updater(
