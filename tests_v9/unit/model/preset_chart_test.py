...
"""Unit tests for PresetChart model in pyatlan_v9."""
...
def test_updater_with_invalid_parameter_raises_value_error(
    ...
    with pytest.raises(ValueError, match=message):
        PresetChart.updater(qualified_name=qualified_name, name=name)


def test_updater():
    """Test updater returns minimal update payload."""
    sut = PresetChart.updater(
        qualified_name=PRESET_CHART_QUALIFIED_NAME, name=PRESET_CHART_NAME
    )
