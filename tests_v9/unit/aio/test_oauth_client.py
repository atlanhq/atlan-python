...
class TestSyncManagerCleanup:
    """Test that the v9 async client only creates an async OAuth manager (no sync manager)"""

    @pytest.mark.asyncio
    async def test_only_async_manager_created(self, clear_env_vars):
        """V9 async client creates only an async OAuth manager, never a sync one"""
        client = AsyncAtlanClient(
            base_url="https://test.atlan.com",
            oauth_client_id="test-client-id",
            oauth_client_secret="test-client-secret",
        ...
        assert client._async_oauth_token_manager is not None
        assert isinstance(client._async_oauth_token_manager, AsyncOAuthTokenManager)

        await client.aclose()

    @pytest.mark.asyncio
    async def test_manager_cleaned_up_on_close(self, clear_env_vars):
        """OAuth manager is cleaned up when async client is closed"""
        client = AsyncAtlanClient(
            base_url="https://test.atlan.com",
            oauth_client_id="test-client-id",
            oauth_client_secret="test-client-secret",
