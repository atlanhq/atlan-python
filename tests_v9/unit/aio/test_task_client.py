...
from pyatlan_v9.client.aio.task import V9AsyncTaskClient as AsyncTaskClient
...
def test_init_when_wrong_class_raises_exception(test_api_caller):
    with pytest.raises(
        ...
        AsyncTaskClient(test_api_caller)
...
@pytest.mark.asyncio
async def test_task_search_get_when_given_request(
    mock_async_api_caller,
    task_search_request,
    task_search_request_json: TaskSearchRequest,
    task_search_response_json: AsyncTaskSearchResponse,
):
    ...
    client = AsyncTaskClient(client=mock_async_api_caller)
    response = await client.search(request=task_search_request)
