...
def _rename_create_update(d):
    """Rename 'create' -> 'creator' and 'update' -> 'updater' keys for v9 sub-clients."""
    out = {}
    for k, v in d.items():
        if k == "create":
            out["creator"] = v
...
V9_TEST_GROUP_CLIENT_METHODS = _rename_create_update(TEST_GROUP_CLIENT_METHODS)
V9_TEST_TOKEN_CLIENT_METHODS = _rename_create_update(TEST_TOKEN_CLIENT_METHODS)
V9_TEST_TYPEDEF_CLIENT_METHODS = _rename_create_update(TEST_TYPEDEF_CLIENT_METHODS)
V9_TEST_USER_CLIENT_METHODS = _rename_create_update(TEST_USER_CLIENT_METHODS)
...
async def test_append_terms_invalid_parameters_raises_error(
    ...
    with pytest.raises(expected_error, match=expected_message):
        await client.asset.append_terms(
...
async def test_replace_terms_invalid_parameters_raises_error(
    ...
    client = AsyncAtlanClient()
    with pytest.raises(expected_error, match=expected_message):
        await client.asset.replace_terms(
            asset_type=asset_type,
            terms=assigned_terms,
            guid=guid,
            qualified_name=qualified_name,
...
async def test_remove_terms_invalid_parameters_raises_error(
    ...
    client = AsyncAtlanClient()
    with pytest.raises(expected_error, match=expected_message):
        await client.asset.remove_terms(
            asset_type=asset_type,
            terms=assigned_terms,
            guid=guid,
            qualified_name=qualified_name,
...
async def test_user_create(
    ...
    async_client._caches["role"] = mock_role_cache

    test_users = [AtlanUser.creator(email="test@test.com", role_name="$member")]
    response = await client.creator(users=test_users)
...
async def test_user_create_with_info(
    ...
    mock_role_cache.get_id_for_name.return_value = test_role_id
    test_users = [AtlanUser.creator(email="test@test.com", role_name="$member")]
    response = await client.creator(users=test_users, return_info=True)
...
async def test_typedef_get_by_name(mock_async_api_caller, type_def_get_by_name_json):
    ...
    assert response == msgspec.convert(type_def_get_by_name_json, EnumDef)
    assert mock_async_api_caller._call_api.call_count == 1
    mock_async_api_caller.reset_mock()
...
@pytest.mark.parametrize("method, params", V9_TEST_ASSET_CLIENT_METHODS_ASYNC.items())
@pytest.mark.asyncio
...
@pytest.mark.parametrize("method, params", V9_TEST_GROUP_CLIENT_METHODS.items())
@pytest.mark.asyncio
...
@pytest.mark.parametrize("method, params", V9_TEST_TOKEN_CLIENT_METHODS.items())
@pytest.mark.asyncio
...
@pytest.mark.parametrize("method, params", V9_TEST_TYPEDEF_CLIENT_METHODS.items())
@pytest.mark.asyncio
...
@pytest.mark.parametrize("method, params", V9_TEST_USER_CLIENT_METHODS.items())
@pytest.mark.asyncio
...
class TestBatch:
    ...
    async def test_term_add(
        ...
        mutated_entities = Mock()
        mock_response = Mock(spec=AssetMutationResponse)
        term_1 = AtlasGlossaryTerm(guid="test-guid1", name="term1")
        term_2 = AtlasGlossaryTerm(guid="test-guid2", name="term2")
...
class TestBulkRequest:
    ...
    def test_process_relationship_attributes(self, glossary, term1, term2, term3):
        ...
        from pyatlan_v9.model.assets.related_entity import (
            SaveSemantic as V9SaveSemantic,
        )
