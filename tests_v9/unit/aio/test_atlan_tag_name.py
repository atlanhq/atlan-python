...
def _assert_asset_tags(asset, is_retranslated=False):
    ...
    assert str(tags[0].type_name) == DELETED_
    ...
    if not is_retranslated:
        assert tags[3].source_tag_attachments == []
    assert str(tags[4].type_name) == DELETED_
...
async def test_asset_tag_name_field_serde_with_translation_async(
    ...
    def _nested_to_purpose(d: dict) -> Purpose:
        """Convert a nested API-format dict to a v9 Purpose."""
