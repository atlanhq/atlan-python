...
async def test_cred_get_when_given_guid(
    ...
    mock_api_caller._call_api.return_value = _to_dict(credential_response)
    result = await client.get(guid="test-id")
    assert result.id == credential_response.id
    assert result.name == credential_response.name
    assert result.host == credential_response.host
    assert result.port == credential_response.port
    cred = (await client.get(guid="test-id")).to_credential()
    assert type(cred).__name__ == "Credential"
...
async def test_cred_test_when_given_cred(
    ...
    assert hasattr(cred_test_response, "message")
...
async def test_creator_success(
    ...
    assert hasattr(response, "id") and hasattr(response, "name")
