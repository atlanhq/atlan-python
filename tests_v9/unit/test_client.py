...
"""
Unit tests for pyatlan_v9 AtlanClient (msgspec.Struct).

Tests ported from tests/unit/test_client.py — configuration, proxy, SSL, headers,
error handling, terms operations, find operations, search/pagination, validation,
batch, bulk request, and asset client tests. Deprecated method tests are excluded
from v9 per migration policy.
"""
...
def load_json(responses_dir, filename):
    """Load JSON test data from a response directory."""
...
def test_append_terms_invalid_parameters_raises_error(
    ...
    """Test that append_terms raises error with invalid parameter combinations."""
    client = AtlanClient()
...
def test_append_terms_asset_retrieval_errors(
    ...
    """Test that append_terms raises appropriate errors for asset retrieval failures."""
...
def test_append_with_valid_guid_and_no_terms_returns_asset():
    """Test that append_terms with empty terms list returns asset unchanged."""
    asset_type = Table
    table = Table()
...
def test_append_with_valid_guid_when_no_terms_present_returns_asset_with_given_terms():
    """Test that append_terms adds new terms when asset has no existing terms."""
    asset_type = Table
    table = Table()
    table.name = "table-test"
    table.qualified_name = "table_qn"
...
def test_append_with_valid_guid_when_terms_present_returns_asset_with_combined_terms():
    """Test that append_terms combines existing and new terms."""
    asset_type = Table
    table = Table()
    table.name = "table-test"
    table.qualified_name = "table_qn"
...
def test_replace_terms_invalid_parameters_raises_error(
    ...
    """Test that replace_terms raises error with invalid parameter combinations."""
    client = AtlanClient()
    with pytest.raises(expected_error, match=expected_message):
...
def test_replace_terms_asset_retrieval_errors(
    ...
    """Test that replace_terms raises appropriate errors for asset retrieval failures."""
    mock_execute.return_value.current_page = lambda: mock_results
    client = AtlanClient()
    with pytest.raises(expected_error, match=expected_message):
        client.asset.replace_terms(
...
def test_replace_terms():
    """Test that replace_terms replaces existing terms with new ones."""
    asset_type = Table
    table = Table()
    table.name = "table-test"
    table.qualified_name = "table_qn"
...
def test_remove_terms_invalid_parameters_raises_error(
    ...
    """Test that remove_terms raises error with invalid parameter combinations."""
    client = AtlanClient()
    with pytest.raises(expected_error, match=expected_message):
...
def test_remove_terms_asset_retrieval_errors(
    ...
    """Test that remove_terms raises appropriate errors for asset retrieval failures."""
    mock_execute.return_value.current_page = lambda: mock_results
    client = AtlanClient()
    with pytest.raises(expected_error, match=expected_message):
        client.asset.remove_terms(
...
def test_remove_with_valid_guid_when_terms_present_returns_asset_with_terms_removed():
    """Test that remove_terms removes specified terms while keeping others."""
    asset_type = Table
    table = Table()
    table.name = "table-test"
    table.qualified_name = "table_qn"
    ...
    with patch("pyatlan.model.fluent_search.FluentSearch.execute") as mock_execute:
        with patch("pyatlan.client.asset.AssetClient.save") as mock_save:
            ...
            assert existing_term not in updated_terms
...
def test_find_glossary_by_name_with_bad_values_raises_value_error(
    ...
    """Test that find_glossary_by_name raises ValueError for bad inputs."""
...
def test_find_category_fast_by_name_with_bad_values_raises_value_error(
    ...
    """Test that find_category_fast_by_name raises ValueError for bad inputs."""
    with pytest.raises(ValueError, match=message):
...
def test_find_category_fast_by_name_when_non_category_found_raises_not_found_error(
    ...
    """Test that find_category_fast_by_name raises NotFoundError when wrong type found."""
    mock_search.return_value.count = 1
    mock_search.return_value.current_page.return_value = [Table()]
...
def test_find_term_fast_by_name_with_bad_values_raises_value_error(
    ...
    """Test that find_term_fast_by_name raises ValueError for bad inputs."""
    with pytest.raises(ValueError, match=message):
...
def test_find_term_fast_by_name_when_non_term_found_raises_not_found_error(
    ...
    """Test that find_term_fast_by_name raises NotFoundError when wrong type found."""
    mock_search.return_value.count = 1
    mock_search.return_value.current_page.return_value = [Table()]
...
def test_search_log_most_recent_viewers(mock_api_caller, sl_most_recent_viewers_json):
    """Test search log most recent viewers parsing."""
...
def test_search_log_most_viewed_assets(mock_api_caller, sl_most_viewed_assets_json):
    """Test search log most viewed assets parsing."""
    client = SearchLogClient(mock_api_caller)
...
def test_search_log_views_by_guid(mock_api_caller, sl_detailed_log_entries_json):
    """Test search log views by GUID with detailed log entries."""
    client = SearchLogClient(mock_api_caller)
...
# ---------------------------------------------------------------------------
# Group/User pagination tests
# ---------------------------------------------------------------------------


def test_group_get_pagination(mock_api_caller, group_list_json):
    """Test group get with pagination returns correct results."""
...
def test_group_get_members_pagination(mock_api_caller, group_members_json):
    """Test group get_members with pagination returns correct results."""
    client = GroupClient(mock_api_caller)
    last_page_response = {"totalRecord": 3, "filterRecord": 3, "records": None}
...
def test_user_list_pagination(mock_api_caller, user_list_json):
    """Test user list with pagination returns correct results."""
...
def test_user_groups_pagination(mock_api_caller, user_groups_json):
    """Test user groups with pagination returns correct results."""
    client = UserClient(mock_api_caller)
...
def test_type_name_in_asset_search_bool_filter(mock_api_caller):
    """Test that type name filter is correctly added/not added in search bool filter."""
...
def test_type_name_in_asset_search_bool_must(mock_api_caller):
    """Test that type name filter is correctly added/not added in search bool must."""
    # When the type name is not present in the request
...
def _assert_search_results(results, response_json, sorts, bulk=False):
    """Helper to assert index search results."""
...
# ---------------------------------------------------------------------------
# Asset get_by_guid / retrieve_minimal tests
# ---------------------------------------------------------------------------
...
def test_user_create_with_info(mock_api_caller, mock_role_cache, user_list_json):
    """Test user creation with return_info=True returns user info."""
    test_role_id = "role-guid-123"
    client = UserClient(mock_api_caller)
    client._client.role_cache = mock_role_cache
    mock_api_caller._call_api.side_effect = [
...
def test_typedef_get_by_name(mock_api_caller, type_def_get_by_name_json):
    """Test typedef get_by_name returns correct EnumDef."""
...
def test_typedef_get_by_name_unsupported_category(mock_api_caller):
    """Test typedef get_by_name raises error for unsupported category."""
    client = TypeDefClient(mock_api_caller)
...
def test_typedef_get_by_name_invalid_response(mock_api_caller):
    """Test typedef get_by_name raises error for invalid response types."""
    client = TypeDefClient(mock_api_caller)
...
# ---------------------------------------------------------------------------
# Asset client missing glossary GUID tests
# ---------------------------------------------------------------------------
...
def test_asset_client_missing_glossary_guid_raises_invalid_request_error(
    ...
    """Test that asset client methods raise error when glossary_guid is missing for glossary types."""
    client = AtlanClient()
...
# ---------------------------------------------------------------------------
# Client methods validation error tests
# ---------------------------------------------------------------------------
...
def test_asset_client_methods_validation_error(client, method, params):
    """Test that asset client methods raise ValidationError for invalid parameters."""
...
def test_admin_client_methods_validation_error(client, method, params):
    """Test that admin client methods raise ValidationError for invalid parameters."""
...
def test_audit_client_methods_validation_error(client, method, params):
    """Test that audit client methods raise ValidationError for invalid parameters."""
...
def test_group_client_methods_validation_error(client, method, params):
    """Test that group client methods raise ValidationError for invalid parameters."""
...
def test_role_client_methods_validation_error(client, method, params):
    """Test that role client methods raise ValidationError for invalid parameters."""
...
def test_search_log_client_methods_validation_error(client, method, params):
    """Test that search log client methods raise ValidationError for invalid parameters."""
...
def test_token_client_methods_validation_error(client, method, params):
    """Test that token client methods raise ValidationError for invalid parameters."""
...
def test_typedef_client_methods_validation_error(client, method, params):
    """Test that typedef client methods raise ValidationError for invalid parameters."""
...
def test_user_client_methods_validation_error(client, method, params):
    """Test that user client methods raise ValidationError for invalid parameters."""
...
def test_atlan_call_api_server_error_messages(
    ...
    """Test that server error messages are properly propagated through AtlanError."""
    mock_response = Mock()
...
def test_atlan_call_api_server_error_messages_with_causes(
    ...
    """Test that error messages with causes are properly parsed and raised."""
...
class TestBatch:
    ...
    def test_init(self, mock_atlan_client):
        """Test batch initialization with empty state."""
    ...
    def assert_asset_client_not_called(self, mock_atlan_client, sut):
        """Helper: assert batch has no created/updated/failures."""
    ...
    def test_add_when_capture_failure_true(
        ...
        """Test batch add with capture_failures=True handles successes and failures."""
    ...
    def test_add_when_capture_failure_false_then_exception_raised(
        ...
        """Test batch add with capture_failures=False raises exception immediately."""
        exception = ErrorCode.INVALID_REQUEST_PASSTHROUGH.exception_with_parameters(
            "bad", "stuff", ""
        )
        if custom_metadata_handling == CustomMetadataHandling.IGNORE:
...
def test_atlan_client_headers(client: AtlanClient):
    """Test that all required HTTP headers are set on the client session."""
...
def test_atlan_client_proxy_from_environment_variables(
    ...
    """Test that proxy configuration is picked up from environment variables."""
    monkeypatch.setenv("ATLAN_BASE_URL", "https://test.atlan.com")
    monkeypatch.setenv("ATLAN_API_KEY", "test-api-key")
...
def test_atlan_client_no_proxy_when_no_env_vars(monkeypatch):
    """Test that no proxy is configured when no env vars are set."""
    monkeypatch.setenv("ATLAN_BASE_URL", "https://test.atlan.com")
    monkeypatch.setenv("ATLAN_API_KEY", "test-api-key")
...
def test_get_all_pagination(group_client, mock_api_caller):
    """Test group get_all pagination returns correct results."""
    mock_page_1 = [
        {"id": "1", "alias": "Group3"},
        {"id": "2", "alias": "Group4"},
    ]
...
def test_get_all_empty_response_with_raw_records(group_client, mock_api_caller):
    """Test group get_all with empty records."""
    mock_page_1 = []
    mock_api_caller._call_api.side_effect = [
        {"records": mock_page_1},
    ]
...
def test_get_all_with_columns(group_client, mock_api_caller):
    """Test group get_all with specific columns requested."""
    mock_page_1 = [
        {"id": "1", "alias": "Group1"},
        {"id": "2", "alias": "Group2"},
    ]
...
def test_get_all_sorting(group_client, mock_api_caller):
    """Test group get_all with sorting parameter."""
    mock_page_1 = [
        {"id": "1", "alias": "Group1"},
        {"id": "2", "alias": "Group2"},
    ]
...
# ---------------------------------------------------------------------------
# FluentSearch-based asset retrieval tests
# ---------------------------------------------------------------------------
...
def test_get_by_guid_type_mismatch_fluent_search(mock_api_caller):
    """Test that get_by_guid raises error when returned asset type doesn't match."""
    guid = "123"
...
def test_get_by_qualified_name_type_mismatch(mock_api_caller):
    """Test that get_by_qualified_name raises error when type doesn't match."""
...
def test_get_by_qualified_name_asset_not_found(mock_api_caller):
    """Test that get_by_qualified_name raises error when asset not found."""
    qualified_name = "example_qualified_name"
    asset_type = Table
...
def test_add_dq_rule_schedule(mock_api_caller):
    ...
    updated_table.asset_dq_schedule_time_zone = schedule_time_zone
    updated_table.asset_dq_schedule_crontab = schedule_cron_string
    updated_table.asset_dq_schedule_type = DataQualityScheduleType.CRON
    ...
    with patch.object(
        ...
        with patch.object(
            ...
            assert updated_table.asset_dq_schedule_time_zone == schedule_time_zone
            assert updated_table.asset_dq_schedule_crontab == schedule_cron_string
            assert updated_table.asset_dq_schedule_type == DataQualityScheduleType.CRON
