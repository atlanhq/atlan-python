...
class V9AsyncGroupClient:
    ...
    through the group property of AsyncAtlanClient.
    """
    ...
    async def creator(
        ...
        raw_json = await self._client._call_api(CREATE_GROUP, request_obj=payload)
        return msgspec.convert(raw_json, CreateGroupResponse, strict=False)

    @validate_arguments
    async def updater(self, group: AtlanGroup) -> None:
        """
        ...
        await self._client._call_api(endpoint, request_obj=group)
    ...
    async def get(
        ...
        """
        Retrieves an AsyncGroupResponse which contains a list of the groups defined in Atlan.
        ...
        :returns: an AsyncGroupResponse which contains a list of groups that match the provided criteria
        :raises AtlanError: on any API communication issue
        """
    ...
    async def get_all(
        ...
        """
        Retrieve an AsyncGroupResponse containing a list of all groups defined in Atlan.
        ...
        :param columns: provides columns projection support for groups endpoint
        :returns: an AsyncGroupResponse with all groups based on the parameters; results are iterable.
        """
        return await self.get(offset=offset, limit=limit, sort=sort, columns=columns)
    ...
    async def get_by_name(
        ...
        """
        Retrieves an AsyncGroupResponse containing a list of groups that match the specified string.

        :param alias: name (as it appears in the UI) on which to filter the groups
        :param limit: maximum number of groups to retrieve
        :param offset: starting point for the list of groups when paging
        :returns: an AsyncGroupResponse containing a list of groups whose UI names include the given string; the results are iterable.
        """
    ...
    async def get_members(
        ...
        """
        Retrieves an AsyncUserResponse object which contains a list of the members (users) of a group.
        ...
        :returns: an AsyncUserResponse object which contains a list of users that are members of the group
        :raises AtlanError: on any API communication issue
        """
        if not request:
            request = UserRequest()
    ...
    async def remove_users(
        ...
        await self._client._call_api(endpoint, request_obj=rfgr)
