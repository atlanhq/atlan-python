...
class V9AsyncUserClient:
    ...
    async def creator(
        ...
        :returns: a AsyncUserResponse object which contains the list of details of created users if `return_info` is `True`, otherwise `None`
        """
    ...
    async def get_current(
        ...
        :raises AtlanError: on any API communication issue
        """
        raw_json = await self._client._call_api(GET_CURRENT_USER, None)
        return msgspec.convert(raw_json, UserMinimalResponse, strict=False)
    ...
    async def get(
        ...
        """
        Retrieves a AsyncUserResponse which contains a list of users defined in Atlan.
        ...
        :returns: a AsyncUserResponse which contains a list of users that match the provided criteria
        :raises AtlanError: on any API communication issue
        """
        request = UserRequest(
    ...
    async def get_all(
        ...
        """
        Retrieve a AsyncUserResponse object containing a list of all users defined in Atlan.
        ...
        :returns: a AsyncUserResponse object with all users based on the parameters; results are iterable.
        """
        response: AsyncUserResponse = await self.get(
    ...
    async def get_by_email(
        ...
        """
        Retrieves a AsyncUserResponse object containing a list of users with email addresses that contain the provided email.
        ...
        :returns: a AsyncUserResponse object containing a list of users whose email addresses contain the provided string
        """
        post_filter = '{"email":{"$ilike":"%' + email + '%"}}'
        return await self.get(offset=offset, limit=limit, post_filter=post_filter)
    ...
    async def get_by_emails(
        ...
        """
        Retrieves a AsyncUserResponse object containing a list of users with email addresses that match the provided list of emails.
        ...
        :returns: a AsyncUserResponse object containing a list of users whose email addresses match the provided list
        """
        email_filter = '{"email":{"$in":' + json.dumps(emails or [""]) + "}}"
        return await self.get(offset=offset, limit=limit, post_filter=email_filter)
    ...
    async def get_by_username(self, username: str) -> Optional[AtlanUser]:
        ...
        response = await self.get(offset=0, limit=5, post_filter=post_filter)
    ...
    async def get_by_usernames(
        ...
        """
        Retrieves a AsyncUserResponse object containing a list of users based on their usernames.

        :param usernames: the list of usernames by which to find the users
        :param limit: maximum number of users to retrieve
        :param offset: starting point for the list of users when paginating
        :returns: a AsyncUserResponse object containing list of users with the specified usernames
        """
        username_filter = '{"username":{"$in":' + json.dumps(usernames or [""]) + "}}"
        return await self.get(offset=offset, limit=limit, post_filter=username_filter)
    ...
    async def get_groups(
        ...
        :returns: an AsyncGroupResponse which contains the groups this user belongs to
        :raises AtlanError: on any API communication issue
        """
        if not request:
    ...
    async def _add_as(
        ...
        :raises NotFoundError: if the asset to which to add the API token as a viewer cannot be found
        """
        from pyatlan_v9.client.aio.atlan import client_connection
        ...
        async with client_connection(
            ...
            results = await tmp.asset.search(request)
            ...
            return await tmp.asset.save(to_update)
