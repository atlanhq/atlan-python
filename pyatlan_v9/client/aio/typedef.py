...
class V9AsyncTypeDefClient:
    """
    Async client for operating on type definitions. This class does not need to be
    instantiated directly but can be obtained through the typedef property of AsyncAtlanClient.
    """
    ...
    async def get_all(self) -> TypeDefResponse:
        ...
        """
        raw_json = await self._client._call_api(GET_ALL_TYPE_DEFS, None)
    ...
    async def get_by_name(self, name: str) -> TypeDef:
        ...
        raw_json = await self._client._call_api(endpoint, None)
        return _create_typedef_from_json(raw_json)

    @validate_arguments
    async def creator(self, typedef: TypeDef) -> TypeDefResponse:
        """
