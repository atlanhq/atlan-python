...
class V9AsyncAuditClient:
    """
    Async version of AuditClient that can be used to configure and run a search
    against Atlan's activity log. This class does not need to be instantiated
    directly but can be obtained through the audit property of AsyncAtlanClient.
    """
    ...
    async def search(
        ...
        if AuditSearch.check_for_bulk_search(
            count, criteria, bulk, AsyncAuditSearchResults
        ):
