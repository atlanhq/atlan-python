...
class DataContract(Asset):
    ...
    def validate(self, for_creation: bool = False) -> None:
        """
        Dry-run validation of this DataContract instance.
        ...
        if for_creation:
            ...
            if self.data_contract_asset_latest is UNSET:
                errors.append("data_contract_asset_latest is required for creation")
        if errors:
            raise ValueError(f"DataContract validation failed: {errors}")

    def minimize(self) -> "DataContract":
        """
        Return a minimal copy of this DataContract with only updater-required fields.

        Calls :meth:`validate` first to ensure the instance is valid, then
        returns a new DataContract with only the fields needed for an update
        (qualified_name, name, and any type-specific additional fields).

        Returns:
            A new DataContract instance with only the minimum required fields.
        """
        self.validate()
        return DataContract(qualified_name=self.qualified_name, name=self.name)

    def relate(self) -> "RelatedDataContract":
        """
        Create a :class:`RelatedDataContract` reference from this instance.
        ...
        Returns:
            A RelatedDataContract reference to this asset.
        """
        if self.guid is not UNSET:
            return RelatedDataContract(guid=self.guid)
        return RelatedDataContract(qualified_name=self.qualified_name)
...
def _data_contract_from_nested(nested: DataContractNested) -> DataContract:
    ...
    kwargs.update(_extract_data_contract_attrs(attrs))
...
DataContract.CATALOG_DATASET_GUID = KeywordField(
...
DataContract.GCP_DATAPLEX_ASPECT_TYPE_METADATA_ENTITIES = RelationField(
    "gcpDataplexAspectTypeMetadataEntities"
)
DataContract.MEANINGS = RelationField("meanings")
DataContract.KNOWLEDGE_LINKED_FILES = RelationField("knowledgeLinkedFiles")
