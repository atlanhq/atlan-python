...
class EventStore(Asset):
    ...
    def validate(self, for_creation: bool = False) -> None:
        """
        Dry-run validation of this EventStore instance.
    ...
    def minimize(self) -> "EventStore":
        """
        Return a minimal copy of this EventStore with only updater-required fields.

        Calls :meth:`validate` first to ensure the instance is valid, then
        returns a new EventStore with only the fields needed for an update
        (qualified_name, name, and any type-specific additional fields).

        Returns:
            A new EventStore instance with only the minimum required fields.
        """
        self.validate()
    ...
    def relate(self) -> "RelatedEventStore":
        """
        Create a :class:`RelatedEventStore` reference from this instance.
        ...
        Returns:
            A RelatedEventStore reference to this asset.
        """
    ...
    def to_nested_bytes(self, serde: Serde | None = None) -> bytes:
        ...
        if serde is None:
            serde = get_serde()
        return _event_store_to_nested_bytes(self, serde)

    @staticmethod
    def from_json(json_data: str | bytes, serde: Serde | None = None) -> EventStore:
        """
...
_EVENT_STORE_REL_FIELDS: List[str] = [
    *_ASSET_REL_FIELDS,
...
def _event_store_to_nested(event_store: EventStore) -> EventStoreNested:
    ...
    _populate_event_store_attrs(attrs, event_store)
    # Categorize relationships by save semantic (REPLACE, APPEND, REMOVE)
    replace_rels, append_rels, remove_rels = categorize_relationships(
        event_store, _EVENT_STORE_REL_FIELDS, EventStoreRelationshipAttributes
    )
...
def _event_store_from_nested(nested: EventStoreNested) -> EventStore:
    ...
    kwargs.update(_extract_event_store_attrs(attrs))
...
EventStore.CATALOG_DATASET_GUID = KeywordField(
    "catalogDatasetGuid", "catalogDatasetGuid"
)
EventStore.INPUT_TO_AIRFLOW_TASKS = RelationField("inputToAirflowTasks")
EventStore.OUTPUT_FROM_AIRFLOW_TASKS = RelationField("outputFromAirflowTasks")
EventStore.ANOMALO_CHECKS = RelationField("anomaloChecks")
EventStore.APPLICATION = RelationField("application")
EventStore.APPLICATION_FIELD = RelationField("applicationField")
EventStore.CONTEXT_REPOSITORIES = RelationField("contextRepositories")
EventStore.DATA_CONTRACT_LATEST = RelationField("dataContractLatest")
EventStore.DATA_CONTRACT_LATEST_CERTIFIED = RelationField("dataContractLatestCertified")
EventStore.OUTPUT_PORT_DATA_PRODUCTS = RelationField("outputPortDataProducts")
EventStore.INPUT_PORT_DATA_PRODUCTS = RelationField("inputPortDataProducts")
EventStore.MODEL_IMPLEMENTED_ENTITIES = RelationField("modelImplementedEntities")
EventStore.MODEL_IMPLEMENTED_ATTRIBUTES = RelationField("modelImplementedAttributes")
EventStore.METRICS = RelationField("metrics")
EventStore.DQ_BASE_DATASET_RULES = RelationField("dqBaseDatasetRules")
EventStore.DQ_REFERENCE_DATASET_RULES = RelationField("dqReferenceDatasetRules")
EventStore.GCP_DATAPLEX_ASPECT_TYPE_METADATA_ENTITIES = RelationField(
    "gcpDataplexAspectTypeMetadataEntities"
)
EventStore.MEANINGS = RelationField("meanings")
EventStore.KNOWLEDGE_LINKED_FILES = RelationField("knowledgeLinkedFiles")
EventStore.MC_MONITORS = RelationField("mcMonitors")
EventStore.MC_INCIDENTS = RelationField("mcIncidents")
EventStore.PARTIAL_CHILD_FIELDS = RelationField("partialChildFields")
EventStore.PARTIAL_CHILD_OBJECTS = RelationField("partialChildObjects")
EventStore.INPUT_TO_PROCESSES = RelationField("inputToProcesses")
EventStore.OUTPUT_FROM_PROCESSES = RelationField("outputFromProcesses")
EventStore.USER_DEF_RELATIONSHIP_TO = RelationField("userDefRelationshipTo")
EventStore.USER_DEF_RELATIONSHIP_FROM = RelationField("userDefRelationshipFrom")
EventStore.FILES = RelationField("files")
EventStore.LINKS = RelationField("links")
EventStore.README = RelationField("readme")
EventStore.SCHEMA_REGISTRY_SUBJECTS = RelationField("schemaRegistrySubjects")
EventStore.SODA_CHECKS = RelationField("sodaChecks")
EventStore.INPUT_TO_SPARK_JOBS = RelationField("inputToSparkJobs")
EventStore.OUTPUT_FROM_SPARK_JOBS = RelationField("outputFromSparkJobs")
