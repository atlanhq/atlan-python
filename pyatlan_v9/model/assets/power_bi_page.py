...
from .power_bi_related import RelatedPowerBIPage, RelatedPowerBIReport
...
class PowerBIPage(Asset):
    ...
    REPORT_QUALIFIED_NAME: ClassVar[Any] = None
    ...
    def validate(self, for_creation: bool = False) -> None:
        """
        Dry-run validation of this PowerBIPage instance.
        ...
        if for_creation:
            ...
            if self.report is UNSET:
                errors.append("report is required for creation")
            if self.report_qualified_name is UNSET:
                errors.append("report_qualified_name is required for creation")
            if self.workspace_qualified_name is UNSET:
                errors.append("workspace_qualified_name is required for creation")
        if errors:
            raise ValueError(f"PowerBIPage validation failed: {errors}")

    def minimize(self) -> "PowerBIPage":
        """
        Return a minimal copy of this PowerBIPage with only updater-required fields.

        Calls :meth:`validate` first to ensure the instance is valid, then
        returns a new PowerBIPage with only the fields needed for an update
        (qualified_name, name, and any type-specific additional fields).

        Returns:
            A new PowerBIPage instance with only the minimum required fields.
        """
        self.validate()
        return PowerBIPage(qualified_name=self.qualified_name, name=self.name)

    def relate(self) -> "RelatedPowerBIPage":
        """
        Create a :class:`RelatedPowerBIPage` reference from this instance.
        ...
        Returns:
            A RelatedPowerBIPage reference to this asset.
        """
        if self.guid is not UNSET:
            return RelatedPowerBIPage(guid=self.guid)
        return RelatedPowerBIPage(qualified_name=self.qualified_name)
    ...
    def to_nested_bytes(self, serde: Serde | None = None) -> bytes:
        ...
        if serde is None:
            serde = get_serde()
        return _power_bi_page_to_nested_bytes(self, serde)

    @staticmethod
    def from_json(json_data: str | bytes, serde: Serde | None = None) -> PowerBIPage:
        """
...
_POWER_BI_PAGE_REL_FIELDS: List[str] = [
    *_ASSET_REL_FIELDS,
...
def _populate_power_bi_page_attrs(
    ...
    attrs.report_qualified_name = obj.report_qualified_name
...
def _power_bi_page_to_nested(power_bi_page: PowerBIPage) -> PowerBIPageNested:
    ...
    _populate_power_bi_page_attrs(attrs, power_bi_page)
    # Categorize relationships by save semantic (REPLACE, APPEND, REMOVE)
    replace_rels, append_rels, remove_rels = categorize_relationships(
        power_bi_page, _POWER_BI_PAGE_REL_FIELDS, PowerBIPageRelationshipAttributes
    )
    return PowerBIPageNested(
        ...
        depth=power_bi_page.depth,
        immediate_upstream=power_bi_page.immediate_upstream,
        immediate_downstream=power_bi_page.immediate_downstream,
        attributes=attrs,
...
def _power_bi_page_from_nested(nested: PowerBIPageNested) -> PowerBIPage:
    ...
    return PowerBIPage(
        ...
        **_extract_power_bi_page_attrs(attrs),
...
PowerBIPage.CATALOG_DATASET_GUID = KeywordField(
...
PowerBIPage.APPLICATION = RelationField("application")
PowerBIPage.APPLICATION_FIELD = RelationField("applicationField")
PowerBIPage.CONTEXT_REPOSITORIES = RelationField("contextRepositories")
PowerBIPage.DATA_CONTRACT_LATEST = RelationField("dataContractLatest")
PowerBIPage.DATA_CONTRACT_LATEST_CERTIFIED = RelationField(
    "dataContractLatestCertified"
)
PowerBIPage.OUTPUT_PORT_DATA_PRODUCTS = RelationField("outputPortDataProducts")
PowerBIPage.INPUT_PORT_DATA_PRODUCTS = RelationField("inputPortDataProducts")
PowerBIPage.MODEL_IMPLEMENTED_ENTITIES = RelationField("modelImplementedEntities")
PowerBIPage.MODEL_IMPLEMENTED_ATTRIBUTES = RelationField("modelImplementedAttributes")
PowerBIPage.METRICS = RelationField("metrics")
PowerBIPage.DQ_BASE_DATASET_RULES = RelationField("dqBaseDatasetRules")
PowerBIPage.DQ_REFERENCE_DATASET_RULES = RelationField("dqReferenceDatasetRules")
PowerBIPage.GCP_DATAPLEX_ASPECT_TYPE_METADATA_ENTITIES = RelationField(
    "gcpDataplexAspectTypeMetadataEntities"
)
PowerBIPage.MEANINGS = RelationField("meanings")
PowerBIPage.KNOWLEDGE_LINKED_FILES = RelationField("knowledgeLinkedFiles")
PowerBIPage.MC_MONITORS = RelationField("mcMonitors")
PowerBIPage.MC_INCIDENTS = RelationField("mcIncidents")
PowerBIPage.PARTIAL_CHILD_FIELDS = RelationField("partialChildFields")
PowerBIPage.PARTIAL_CHILD_OBJECTS = RelationField("partialChildObjects")
PowerBIPage.REPORT = RelationField("report")
PowerBIPage.INPUT_TO_PROCESSES = RelationField("inputToProcesses")
PowerBIPage.OUTPUT_FROM_PROCESSES = RelationField("outputFromProcesses")
PowerBIPage.USER_DEF_RELATIONSHIP_TO = RelationField("userDefRelationshipTo")
PowerBIPage.USER_DEF_RELATIONSHIP_FROM = RelationField("userDefRelationshipFrom")
PowerBIPage.FILES = RelationField("files")
PowerBIPage.LINKS = RelationField("links")
PowerBIPage.README = RelationField("readme")
PowerBIPage.SCHEMA_REGISTRY_SUBJECTS = RelationField("schemaRegistrySubjects")
PowerBIPage.SODA_CHECKS = RelationField("sodaChecks")
PowerBIPage.INPUT_TO_SPARK_JOBS = RelationField("inputToSparkJobs")
PowerBIPage.OUTPUT_FROM_SPARK_JOBS = RelationField("outputFromSparkJobs")
