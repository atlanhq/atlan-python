...
"""
Iceberg asset model with flattened inheritance.

This module provides:
- Iceberg: Flat asset class (easy to use)
- IcebergAttributes: Nested attributes struct (extends AssetAttributes)
- IcebergNested: Nested API format struct
"""
...
class Iceberg(Asset):
    ...
    def to_nested_bytes(self, serde: Serde | None = None) -> bytes:
        ...
        if serde is None:
            serde = get_serde()
        return _iceberg_to_nested_bytes(self, serde)

    @staticmethod
    def from_json(json_data: str | bytes, serde: Serde | None = None) -> Iceberg:
        """
        ...
        if serde is None:
            serde = get_serde()
        return _iceberg_from_nested_bytes(json_data, serde)
...
class IcebergNested(AssetNested):
    ...
    attributes: Union[IcebergAttributes, UnsetType] = UNSET
    relationship_attributes: Union[IcebergRelationshipAttributes, UnsetType] = UNSET
    append_relationship_attributes: Union[IcebergRelationshipAttributes, UnsetType] = (
        UNSET
    )
    remove_relationship_attributes: Union[IcebergRelationshipAttributes, UnsetType] = (
        UNSET
    )
...
def _iceberg_to_nested(iceberg: Iceberg) -> IcebergNested:
    """Convert flat Iceberg to nested format."""
    attrs = IcebergAttributes()
    _populate_iceberg_attrs(attrs, iceberg)
    # Categorize relationships by save semantic (REPLACE, APPEND, REMOVE)
    replace_rels, append_rels, remove_rels = categorize_relationships(
        iceberg, _ICEBERG_REL_FIELDS, IcebergRelationshipAttributes
    )
    return IcebergNested(
        guid=iceberg.guid,
        type_name=iceberg.type_name,
        status=iceberg.status,
        version=iceberg.version,
        create_time=iceberg.create_time,
        update_time=iceberg.update_time,
        created_by=iceberg.created_by,
        updated_by=iceberg.updated_by,
        classifications=iceberg.classifications,
        classification_names=iceberg.classification_names,
        meanings=iceberg.meanings,
        labels=iceberg.labels,
        business_attributes=iceberg.business_attributes,
        custom_attributes=iceberg.custom_attributes,
        pending_tasks=iceberg.pending_tasks,
        proxy=iceberg.proxy,
...
def _iceberg_from_nested(nested: IcebergNested) -> Iceberg:
    ...
    kwargs.update(_extract_iceberg_attrs(attrs))
...
def _iceberg_from_nested_bytes(data: bytes, serde: Serde) -> Iceberg:
...
Iceberg.SQL_HAS_AI_INSIGHTS = BooleanField("sqlHasAiInsights", "sqlHasAiInsights")
Iceberg.SQL_AI_INSIGHTS_LAST_ANALYZED_AT = NumericField(
    "sqlAiInsightsLastAnalyzedAt", "sqlAiInsightsLastAnalyzedAt"
)
Iceberg.SQL_AI_INSIGHTS_POPULAR_BUSINESS_QUESTION_COUNT = NumericField(
    "sqlAiInsightsPopularBusinessQuestionCount",
    "sqlAiInsightsPopularBusinessQuestionCount",
)
Iceberg.SQL_AI_INSIGHTS_POPULAR_JOIN_COUNT = NumericField(
    "sqlAiInsightsPopularJoinCount", "sqlAiInsightsPopularJoinCount"
)
Iceberg.SQL_AI_INSIGHTS_POPULAR_FILTER_COUNT = NumericField(
    "sqlAiInsightsPopularFilterCount", "sqlAiInsightsPopularFilterCount"
)
Iceberg.SQL_AI_INSIGHTS_RELATIONSHIP_COUNT = NumericField(
    "sqlAiInsightsRelationshipCount", "sqlAiInsightsRelationshipCount"
)
Iceberg.SQL_COALESCE_LAST_RUN_STATUS = KeywordField(
    "sqlCoalesceLastRunStatus", "sqlCoalesceLastRunStatus"
)
Iceberg.SQL_COALESCE_NODE_STATUS = KeywordField(
    "sqlCoalesceNodeStatus", "sqlCoalesceNodeStatus"
)
Iceberg.SQL_COALESCE_LAST_RUN_AT = NumericField(
    "sqlCoalesceLastRunAt", "sqlCoalesceLastRunAt"
)
Iceberg.SQL_COALESCE_NODE_TYPE = KeywordField(
    "sqlCoalesceNodeType", "sqlCoalesceNodeType"
)
Iceberg.SQL_COALESCE_ENVIRONMENT_ID = KeywordField(
    "sqlCoalesceEnvironmentId", "sqlCoalesceEnvironmentId"
)
Iceberg.SQL_COALESCE_ENVIRONMENT_NAME = KeywordTextField(
    "sqlCoalesceEnvironmentName",
    "sqlCoalesceEnvironmentName",
    "sqlCoalesceEnvironmentName.text",
)
Iceberg.SQL_COALESCE_PROJECT_ID = KeywordField(
    "sqlCoalesceProjectId", "sqlCoalesceProjectId"
)
Iceberg.SQL_COALESCE_PROJECT_NAME = KeywordTextField(
    "sqlCoalesceProjectName", "sqlCoalesceProjectName", "sqlCoalesceProjectName.text"
)
Iceberg.SQL_SHARE_QUALIFIED_NAMES = KeywordField(
    "sqlShareQualifiedNames", "sqlShareQualifiedNames"
)
Iceberg.CATALOG_DATASET_GUID = KeywordField("catalogDatasetGuid", "catalogDatasetGuid")
...
Iceberg.ANOMALO_CHECKS = RelationField("anomaloChecks")
Iceberg.APPLICATION = RelationField("application")
Iceberg.APPLICATION_FIELD = RelationField("applicationField")
Iceberg.CONTEXT_REPOSITORIES = RelationField("contextRepositories")
Iceberg.DATA_CONTRACT_LATEST = RelationField("dataContractLatest")
Iceberg.DATA_CONTRACT_LATEST_CERTIFIED = RelationField("dataContractLatestCertified")
Iceberg.OUTPUT_PORT_DATA_PRODUCTS = RelationField("outputPortDataProducts")
Iceberg.INPUT_PORT_DATA_PRODUCTS = RelationField("inputPortDataProducts")
Iceberg.MODEL_IMPLEMENTED_ENTITIES = RelationField("modelImplementedEntities")
Iceberg.MODEL_IMPLEMENTED_ATTRIBUTES = RelationField("modelImplementedAttributes")
Iceberg.METRICS = RelationField("metrics")
Iceberg.DQ_BASE_DATASET_RULES = RelationField("dqBaseDatasetRules")
Iceberg.DQ_REFERENCE_DATASET_RULES = RelationField("dqReferenceDatasetRules")
Iceberg.DBT_MODELS = RelationField("dbtModels")
Iceberg.SQL_DBT_MODELS = RelationField("sqlDbtModels")
Iceberg.DBT_TESTS = RelationField("dbtTests")
Iceberg.DBT_SOURCES = RelationField("dbtSources")
Iceberg.SQL_DBT_SOURCES = RelationField("sqlDBTSources")
Iceberg.DBT_SEED_ASSETS = RelationField("dbtSeedAssets")
Iceberg.GCP_DATAPLEX_ASPECT_TYPE_METADATA_ENTITIES = RelationField(
    "gcpDataplexAspectTypeMetadataEntities"
)
Iceberg.MEANINGS = RelationField("meanings")
Iceberg.KNOWLEDGE_LINKED_FILES = RelationField("knowledgeLinkedFiles")
Iceberg.MC_MONITORS = RelationField("mcMonitors")
Iceberg.MC_INCIDENTS = RelationField("mcIncidents")
Iceberg.PARTIAL_CHILD_FIELDS = RelationField("partialChildFields")
Iceberg.PARTIAL_CHILD_OBJECTS = RelationField("partialChildObjects")
Iceberg.INPUT_TO_PROCESSES = RelationField("inputToProcesses")
Iceberg.OUTPUT_FROM_PROCESSES = RelationField("outputFromProcesses")
Iceberg.USER_DEF_RELATIONSHIP_TO = RelationField("userDefRelationshipTo")
Iceberg.USER_DEF_RELATIONSHIP_FROM = RelationField("userDefRelationshipFrom")
Iceberg.FILES = RelationField("files")
Iceberg.LINKS = RelationField("links")
Iceberg.README = RelationField("readme")
Iceberg.SCHEMA_REGISTRY_SUBJECTS = RelationField("schemaRegistrySubjects")
Iceberg.SNOWFLAKE_SEMANTIC_LOGICAL_TABLES = RelationField(
    "snowflakeSemanticLogicalTables"
)
Iceberg.SODA_CHECKS = RelationField("sodaChecks")
Iceberg.INPUT_TO_SPARK_JOBS = RelationField("inputToSparkJobs")
Iceberg.OUTPUT_FROM_SPARK_JOBS = RelationField("outputFromSparkJobs")
Iceberg.SQL_INSIGHT_OUTGOING_JOINS = RelationField("sqlInsightOutgoingJoins")
Iceberg.SQL_INSIGHT_INCOMING_JOINS = RelationField("sqlInsightIncomingJoins")
Iceberg.SQL_INSIGHT_BUSINESS_QUESTIONS = RelationField("sqlInsightBusinessQuestions")
