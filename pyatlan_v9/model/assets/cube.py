...
from .cube_related import RelatedCube, RelatedCubeDimension
...
class Cube(Asset):
    ...
    CUBE_DIMENSION_COUNT: ClassVar[Any] = None
    ...
    def validate(self, for_creation: bool = False) -> None:
        """
        Dry-run validation of this Cube instance.
        ...
        if errors:
            raise ValueError(f"Cube validation failed: {errors}")

    def minimize(self) -> "Cube":
        """
        Return a minimal copy of this Cube with only updater-required fields.

        Calls :meth:`validate` first to ensure the instance is valid, then
        returns a new Cube with only the fields needed for an update
        (qualified_name, name, and any type-specific additional fields).

        Returns:
            A new Cube instance with only the minimum required fields.
        """
        self.validate()
        return Cube(qualified_name=self.qualified_name, name=self.name)

    def relate(self) -> "RelatedCube":
        """
        Create a :class:`RelatedCube` reference from this instance.
        ...
        Returns:
            A RelatedCube reference to this asset.
        """
        if self.guid is not UNSET:
            return RelatedCube(guid=self.guid)
        return RelatedCube(qualified_name=self.qualified_name)
    ...
    def to_nested_bytes(self, serde: Serde | None = None) -> bytes:
        ...
        if serde is None:
            serde = get_serde()
        return _cube_to_nested_bytes(self, serde)

    @staticmethod
    def from_json(json_data: str | bytes, serde: Serde | None = None) -> Cube:
        """
...
def _populate_cube_attrs(attrs: CubeAttributes, obj: Cube) -> None:
    ...
    attrs.cube_dimension_count = obj.cube_dimension_count
...
def _extract_cube_attrs(attrs: CubeAttributes) -> dict:
    ...
    result["cube_dimension_count"] = attrs.cube_dimension_count
...
def _cube_to_nested(cube: Cube) -> CubeNested:
    ...
    _populate_cube_attrs(attrs, cube)
    # Categorize relationships by save semantic (REPLACE, APPEND, REMOVE)
    replace_rels, append_rels, remove_rels = categorize_relationships(
        cube, _CUBE_REL_FIELDS, CubeRelationshipAttributes
    )
    return CubeNested(
        guid=cube.guid,
        ...
        depth=cube.depth,
        immediate_upstream=cube.immediate_upstream,
        immediate_downstream=cube.immediate_downstream,
        attributes=attrs,
...
def _cube_from_nested(nested: CubeNested) -> Cube:
    ...
    kwargs.update(_extract_cube_attrs(attrs))
...
Cube.CATALOG_DATASET_GUID = KeywordField("catalogDatasetGuid", "catalogDatasetGuid")
...
Cube.APPLICATION = RelationField("application")
Cube.APPLICATION_FIELD = RelationField("applicationField")
Cube.CONTEXT_REPOSITORIES = RelationField("contextRepositories")
Cube.DATA_CONTRACT_LATEST = RelationField("dataContractLatest")
Cube.DATA_CONTRACT_LATEST_CERTIFIED = RelationField("dataContractLatestCertified")
Cube.OUTPUT_PORT_DATA_PRODUCTS = RelationField("outputPortDataProducts")
Cube.INPUT_PORT_DATA_PRODUCTS = RelationField("inputPortDataProducts")
Cube.MODEL_IMPLEMENTED_ENTITIES = RelationField("modelImplementedEntities")
Cube.MODEL_IMPLEMENTED_ATTRIBUTES = RelationField("modelImplementedAttributes")
Cube.METRICS = RelationField("metrics")
Cube.DQ_BASE_DATASET_RULES = RelationField("dqBaseDatasetRules")
Cube.DQ_REFERENCE_DATASET_RULES = RelationField("dqReferenceDatasetRules")
Cube.GCP_DATAPLEX_ASPECT_TYPE_METADATA_ENTITIES = RelationField(
    "gcpDataplexAspectTypeMetadataEntities"
)
Cube.MEANINGS = RelationField("meanings")
Cube.KNOWLEDGE_LINKED_FILES = RelationField("knowledgeLinkedFiles")
Cube.MC_MONITORS = RelationField("mcMonitors")
Cube.MC_INCIDENTS = RelationField("mcIncidents")
Cube.CUBE_DIMENSIONS = RelationField("cubeDimensions")
Cube.PARTIAL_CHILD_FIELDS = RelationField("partialChildFields")
Cube.PARTIAL_CHILD_OBJECTS = RelationField("partialChildObjects")
Cube.INPUT_TO_PROCESSES = RelationField("inputToProcesses")
Cube.OUTPUT_FROM_PROCESSES = RelationField("outputFromProcesses")
Cube.USER_DEF_RELATIONSHIP_TO = RelationField("userDefRelationshipTo")
Cube.USER_DEF_RELATIONSHIP_FROM = RelationField("userDefRelationshipFrom")
Cube.FILES = RelationField("files")
Cube.LINKS = RelationField("links")
Cube.README = RelationField("readme")
Cube.SCHEMA_REGISTRY_SUBJECTS = RelationField("schemaRegistrySubjects")
Cube.SODA_CHECKS = RelationField("sodaChecks")
Cube.INPUT_TO_SPARK_JOBS = RelationField("inputToSparkJobs")
Cube.OUTPUT_FROM_SPARK_JOBS = RelationField("outputFromSparkJobs")
