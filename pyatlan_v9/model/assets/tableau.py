...
class Tableau(Asset):
    ...
    def validate(self, for_creation: bool = False) -> None:
        """
        Dry-run validation of this Tableau instance.
    ...
    def minimize(self) -> "Tableau":
        """
        Return a minimal copy of this Tableau with only updater-required fields.

        Calls :meth:`validate` first to ensure the instance is valid, then
        returns a new Tableau with only the fields needed for an update
        (qualified_name, name, and any type-specific additional fields).

        Returns:
            A new Tableau instance with only the minimum required fields.
        """
        self.validate()
    ...
    def relate(self) -> "RelatedTableau":
        """
        Create a :class:`RelatedTableau` reference from this instance.
        ...
        Returns:
            A RelatedTableau reference to this asset.
        """
    ...
    def to_nested_bytes(self, serde: Serde | None = None) -> bytes:
        ...
        if serde is None:
            serde = get_serde()
        return _tableau_to_nested_bytes(self, serde)

    @staticmethod
    def from_json(json_data: str | bytes, serde: Serde | None = None) -> Tableau:
        """
...
def _tableau_to_nested(tableau: Tableau) -> TableauNested:
    ...
    _populate_tableau_attrs(attrs, tableau)
    # Categorize relationships by save semantic (REPLACE, APPEND, REMOVE)
    replace_rels, append_rels, remove_rels = categorize_relationships(
        tableau, _TABLEAU_REL_FIELDS, TableauRelationshipAttributes
    )
...
def _tableau_from_nested(nested: TableauNested) -> Tableau:
    ...
    kwargs.update(_extract_tableau_attrs(attrs))
    kwargs.update(merged_rels)
    return Tableau(**kwargs)
...
Tableau.TABLEAU_SOURCE_READ_COUNTS = KeywordField(
    "tableauSourceReadCounts", "tableauSourceReadCounts"
)
Tableau.CATALOG_DATASET_GUID = KeywordField("catalogDatasetGuid", "catalogDatasetGuid")
...
Tableau.APPLICATION = RelationField("application")
Tableau.APPLICATION_FIELD = RelationField("applicationField")
Tableau.CONTEXT_REPOSITORIES = RelationField("contextRepositories")
Tableau.DATA_CONTRACT_LATEST = RelationField("dataContractLatest")
Tableau.DATA_CONTRACT_LATEST_CERTIFIED = RelationField("dataContractLatestCertified")
Tableau.OUTPUT_PORT_DATA_PRODUCTS = RelationField("outputPortDataProducts")
Tableau.INPUT_PORT_DATA_PRODUCTS = RelationField("inputPortDataProducts")
Tableau.MODEL_IMPLEMENTED_ENTITIES = RelationField("modelImplementedEntities")
Tableau.MODEL_IMPLEMENTED_ATTRIBUTES = RelationField("modelImplementedAttributes")
Tableau.METRICS = RelationField("metrics")
Tableau.DQ_BASE_DATASET_RULES = RelationField("dqBaseDatasetRules")
Tableau.DQ_REFERENCE_DATASET_RULES = RelationField("dqReferenceDatasetRules")
Tableau.GCP_DATAPLEX_ASPECT_TYPE_METADATA_ENTITIES = RelationField(
    "gcpDataplexAspectTypeMetadataEntities"
)
Tableau.MEANINGS = RelationField("meanings")
Tableau.KNOWLEDGE_LINKED_FILES = RelationField("knowledgeLinkedFiles")
Tableau.MC_MONITORS = RelationField("mcMonitors")
Tableau.MC_INCIDENTS = RelationField("mcIncidents")
Tableau.PARTIAL_CHILD_FIELDS = RelationField("partialChildFields")
Tableau.PARTIAL_CHILD_OBJECTS = RelationField("partialChildObjects")
Tableau.INPUT_TO_PROCESSES = RelationField("inputToProcesses")
Tableau.OUTPUT_FROM_PROCESSES = RelationField("outputFromProcesses")
Tableau.USER_DEF_RELATIONSHIP_TO = RelationField("userDefRelationshipTo")
Tableau.USER_DEF_RELATIONSHIP_FROM = RelationField("userDefRelationshipFrom")
Tableau.FILES = RelationField("files")
Tableau.LINKS = RelationField("links")
Tableau.README = RelationField("readme")
Tableau.SCHEMA_REGISTRY_SUBJECTS = RelationField("schemaRegistrySubjects")
Tableau.SODA_CHECKS = RelationField("sodaChecks")
Tableau.INPUT_TO_SPARK_JOBS = RelationField("inputToSparkJobs")
Tableau.OUTPUT_FROM_SPARK_JOBS = RelationField("outputFromSparkJobs")
