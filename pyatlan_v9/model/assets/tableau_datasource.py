...
class TableauDatasource(Asset):
    ...
    def validate(self, for_creation: bool = False) -> None:
        """
        Dry-run validation of this TableauDatasource instance.
        ...
        if errors:
            raise ValueError(f"TableauDatasource validation failed: {errors}")

    def minimize(self) -> "TableauDatasource":
        """
        Return a minimal copy of this TableauDatasource with only updater-required fields.

        Calls :meth:`validate` first to ensure the instance is valid, then
        returns a new TableauDatasource with only the fields needed for an update
        (qualified_name, name, and any type-specific additional fields).

        Returns:
            A new TableauDatasource instance with only the minimum required fields.
        """
        self.validate()
        return TableauDatasource(qualified_name=self.qualified_name, name=self.name)

    def relate(self) -> "RelatedTableauDatasource":
        """
        Create a :class:`RelatedTableauDatasource` reference from this instance.
        ...
        Returns:
            A RelatedTableauDatasource reference to this asset.
        """
        if self.guid is not UNSET:
            return RelatedTableauDatasource(guid=self.guid)
        return RelatedTableauDatasource(qualified_name=self.qualified_name)
...
def _tableau_datasource_to_nested(
    ...
    return TableauDatasourceNested(
        ...
        depth=tableau_datasource.depth,
        immediate_upstream=tableau_datasource.immediate_upstream,
        immediate_downstream=tableau_datasource.immediate_downstream,
        attributes=attrs,
...
def _tableau_datasource_from_nested(
    ...
    return TableauDatasource(**kwargs)
...
TableauDatasource.TABLEAU_SOURCE_READ_COUNTS = KeywordField(
    "tableauSourceReadCounts", "tableauSourceReadCounts"
)
TableauDatasource.CATALOG_DATASET_GUID = KeywordField(
...
TableauDatasource.CONTEXT_REPOSITORIES = RelationField("contextRepositories")
TableauDatasource.DATA_CONTRACT_LATEST = RelationField("dataContractLatest")
TableauDatasource.DATA_CONTRACT_LATEST_CERTIFIED = RelationField(
...
TableauDatasource.GCP_DATAPLEX_ASPECT_TYPE_METADATA_ENTITIES = RelationField(
    "gcpDataplexAspectTypeMetadataEntities"
)
TableauDatasource.MEANINGS = RelationField("meanings")
TableauDatasource.KNOWLEDGE_LINKED_FILES = RelationField("knowledgeLinkedFiles")
