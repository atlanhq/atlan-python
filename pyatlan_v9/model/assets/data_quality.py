...
class DataQuality(Asset):
    ...
    def validate(self, for_creation: bool = False) -> None:
        """
        Dry-run validation of this DataQuality instance.
        ...
        if errors:
            raise ValueError(f"DataQuality validation failed: {errors}")

    def minimize(self) -> "DataQuality":
        """
        Return a minimal copy of this DataQuality with only updater-required fields.

        Calls :meth:`validate` first to ensure the instance is valid, then
        returns a new DataQuality with only the fields needed for an update
        (qualified_name, name, and any type-specific additional fields).

        Returns:
            A new DataQuality instance with only the minimum required fields.
        """
        self.validate()
        return DataQuality(qualified_name=self.qualified_name, name=self.name)

    def relate(self) -> "RelatedDataQuality":
        """
        Create a :class:`RelatedDataQuality` reference from this instance.
        ...
        Returns:
            A RelatedDataQuality reference to this asset.
        """
        if self.guid is not UNSET:
            return RelatedDataQuality(guid=self.guid)
        return RelatedDataQuality(qualified_name=self.qualified_name)
...
def _data_quality_from_nested(nested: DataQualityNested) -> DataQuality:
    ...
    kwargs.update(_extract_data_quality_attrs(attrs))
...
DataQuality.KNOWLEDGE_LINKED_FILES = RelationField("knowledgeLinkedFiles")
