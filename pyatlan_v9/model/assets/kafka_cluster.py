...
class KafkaCluster(Asset):
    ...
    def validate(self, for_creation: bool = False) -> None:
        """
        Dry-run validation of this KafkaCluster instance.
        ...
        if errors:
            raise ValueError(f"KafkaCluster validation failed: {errors}")

    def minimize(self) -> "KafkaCluster":
        """
        Return a minimal copy of this KafkaCluster with only updater-required fields.

        Calls :meth:`validate` first to ensure the instance is valid, then
        returns a new KafkaCluster with only the fields needed for an update
        (qualified_name, name, and any type-specific additional fields).

        Returns:
            A new KafkaCluster instance with only the minimum required fields.
        """
        self.validate()
        return KafkaCluster(qualified_name=self.qualified_name, name=self.name)

    def relate(self) -> "RelatedKafkaCluster":
        """
        Create a :class:`RelatedKafkaCluster` reference from this instance.
        ...
        Returns:
            A RelatedKafkaCluster reference to this asset.
        """
        if self.guid is not UNSET:
            return RelatedKafkaCluster(guid=self.guid)
        return RelatedKafkaCluster(qualified_name=self.qualified_name)
...
def _kafka_cluster_to_nested(kafka_cluster: KafkaCluster) -> KafkaClusterNested:
    ...
    return KafkaClusterNested(
        ...
        depth=kafka_cluster.depth,
        immediate_upstream=kafka_cluster.immediate_upstream,
        immediate_downstream=kafka_cluster.immediate_downstream,
        attributes=attrs,
...
def _kafka_cluster_from_nested(nested: KafkaClusterNested) -> KafkaCluster:
    ...
    kwargs.update(_extract_kafka_cluster_attrs(attrs))
    kwargs.update(merged_rels)
    return KafkaCluster(**kwargs)
...
KafkaCluster.CONTEXT_REPOSITORIES = RelationField("contextRepositories")
