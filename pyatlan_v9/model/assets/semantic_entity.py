...
from .semantic_related import RelatedSemanticEntity, RelatedSemanticModel
...
class SemanticEntity(Asset):
    ...
    def validate(self, for_creation: bool = False) -> None:
        """
        Dry-run validation of this SemanticEntity instance.
        ...
        if errors:
            raise ValueError(f"SemanticEntity validation failed: {errors}")

    def minimize(self) -> "SemanticEntity":
        """
        Return a minimal copy of this SemanticEntity with only updater-required fields.

        Calls :meth:`validate` first to ensure the instance is valid, then
        returns a new SemanticEntity with only the fields needed for an update
        (qualified_name, name, and any type-specific additional fields).

        Returns:
            A new SemanticEntity instance with only the minimum required fields.
        """
        self.validate()
        return SemanticEntity(qualified_name=self.qualified_name, name=self.name)

    def relate(self) -> "RelatedSemanticEntity":
        """
        Create a :class:`RelatedSemanticEntity` reference from this instance.
        ...
        Returns:
            A RelatedSemanticEntity reference to this asset.
        """
        if self.guid is not UNSET:
            return RelatedSemanticEntity(guid=self.guid)
        return RelatedSemanticEntity(qualified_name=self.qualified_name)
...
def _semantic_entity_from_nested(nested: SemanticEntityNested) -> SemanticEntity:
    ...
    kwargs.update(_extract_semantic_entity_attrs(attrs))
    kwargs.update(merged_rels)
    return SemanticEntity(**kwargs)
...
SemanticEntity.CONTEXT_REPOSITORIES = RelationField("contextRepositories")
...
SemanticEntity.KNOWLEDGE_LINKED_FILES = RelationField("knowledgeLinkedFiles")
