...
class CosmosMongoDB(Asset):
    ...
    def validate(self, for_creation: bool = False) -> None:
        """
        Dry-run validation of this CosmosMongoDB instance.
        ...
        if errors:
            raise ValueError(f"CosmosMongoDB validation failed: {errors}")

    def minimize(self) -> "CosmosMongoDB":
        """
        Return a minimal copy of this CosmosMongoDB with only updater-required fields.

        Calls :meth:`validate` first to ensure the instance is valid, then
        returns a new CosmosMongoDB with only the fields needed for an update
        (qualified_name, name, and any type-specific additional fields).

        Returns:
            A new CosmosMongoDB instance with only the minimum required fields.
        """
        self.validate()
        return CosmosMongoDB(qualified_name=self.qualified_name, name=self.name)

    def relate(self) -> "RelatedCosmosMongoDB":
        """
        Create a :class:`RelatedCosmosMongoDB` reference from this instance.
        ...
        Returns:
            A RelatedCosmosMongoDB reference to this asset.
        """
        if self.guid is not UNSET:
            return RelatedCosmosMongoDB(guid=self.guid)
        return RelatedCosmosMongoDB(qualified_name=self.qualified_name)
    ...
    def to_nested_bytes(self, serde: Serde | None = None) -> bytes:
        ...
        if serde is None:
            serde = get_serde()
        return _cosmos_mongo_db_to_nested_bytes(self, serde)

    @staticmethod
    def from_json(json_data: str | bytes, serde: Serde | None = None) -> CosmosMongoDB:
        """
...
_COSMOS_MONGO_DB_REL_FIELDS: List[str] = [
    *_ASSET_REL_FIELDS,
...
def _cosmos_mongo_db_to_nested(cosmos_mongo_db: CosmosMongoDB) -> CosmosMongoDBNested:
    ...
    return CosmosMongoDBNested(
        ...
        depth=cosmos_mongo_db.depth,
        immediate_upstream=cosmos_mongo_db.immediate_upstream,
        immediate_downstream=cosmos_mongo_db.immediate_downstream,
        attributes=attrs,
...
def _cosmos_mongo_db_from_nested(nested: CosmosMongoDBNested) -> CosmosMongoDB:
    ...
    kwargs.update(_extract_cosmos_mongo_db_attrs(attrs))
    kwargs.update(merged_rels)
    return CosmosMongoDB(**kwargs)
...
CosmosMongoDB.CATALOG_DATASET_GUID = KeywordField(
...
CosmosMongoDB.OUTPUT_PORT_DATA_PRODUCTS = RelationField("outputPortDataProducts")
CosmosMongoDB.INPUT_PORT_DATA_PRODUCTS = RelationField("inputPortDataProducts")
CosmosMongoDB.MODEL_IMPLEMENTED_ENTITIES = RelationField("modelImplementedEntities")
CosmosMongoDB.MODEL_IMPLEMENTED_ATTRIBUTES = RelationField("modelImplementedAttributes")
CosmosMongoDB.METRICS = RelationField("metrics")
CosmosMongoDB.DQ_BASE_DATASET_RULES = RelationField("dqBaseDatasetRules")
CosmosMongoDB.DQ_REFERENCE_DATASET_RULES = RelationField("dqReferenceDatasetRules")
CosmosMongoDB.GCP_DATAPLEX_ASPECT_TYPE_METADATA_ENTITIES = RelationField(
    "gcpDataplexAspectTypeMetadataEntities"
)
CosmosMongoDB.MEANINGS = RelationField("meanings")
CosmosMongoDB.KNOWLEDGE_LINKED_FILES = RelationField("knowledgeLinkedFiles")
CosmosMongoDB.MC_MONITORS = RelationField("mcMonitors")
CosmosMongoDB.MC_INCIDENTS = RelationField("mcIncidents")
CosmosMongoDB.PARTIAL_CHILD_FIELDS = RelationField("partialChildFields")
CosmosMongoDB.PARTIAL_CHILD_OBJECTS = RelationField("partialChildObjects")
CosmosMongoDB.INPUT_TO_PROCESSES = RelationField("inputToProcesses")
CosmosMongoDB.OUTPUT_FROM_PROCESSES = RelationField("outputFromProcesses")
CosmosMongoDB.USER_DEF_RELATIONSHIP_TO = RelationField("userDefRelationshipTo")
CosmosMongoDB.USER_DEF_RELATIONSHIP_FROM = RelationField("userDefRelationshipFrom")
CosmosMongoDB.FILES = RelationField("files")
CosmosMongoDB.LINKS = RelationField("links")
CosmosMongoDB.README = RelationField("readme")
CosmosMongoDB.SCHEMA_REGISTRY_SUBJECTS = RelationField("schemaRegistrySubjects")
CosmosMongoDB.SODA_CHECKS = RelationField("sodaChecks")
CosmosMongoDB.INPUT_TO_SPARK_JOBS = RelationField("inputToSparkJobs")
CosmosMongoDB.OUTPUT_FROM_SPARK_JOBS = RelationField("outputFromSparkJobs")
