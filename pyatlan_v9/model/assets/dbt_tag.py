...
class DbtTag(Asset):
    ...
    def validate(self, for_creation: bool = False) -> None:
        """
        Dry-run validation of this DbtTag instance.
    ...
    def minimize(self) -> "DbtTag":
        """
        Return a minimal copy of this DbtTag with only updater-required fields.

        Calls :meth:`validate` first to ensure the instance is valid, then
        returns a new DbtTag with only the fields needed for an update
        (qualified_name, name, and any type-specific additional fields).

        Returns:
            A new DbtTag instance with only the minimum required fields.
        """
        self.validate()
    ...
    def relate(self) -> "RelatedDbtTag":
        """
        Create a :class:`RelatedDbtTag` reference from this instance.
        ...
        Returns:
            A RelatedDbtTag reference to this asset.
        """
    ...
    def to_nested_bytes(self, serde: Serde | None = None) -> bytes:
        ...
        if serde is None:
            serde = get_serde()
        return _dbt_tag_to_nested_bytes(self, serde)
...
def _dbt_tag_to_nested(dbt_tag: DbtTag) -> DbtTagNested:
    ...
    _populate_dbt_tag_attrs(attrs, dbt_tag)
    # Categorize relationships by save semantic (REPLACE, APPEND, REMOVE)
    replace_rels, append_rels, remove_rels = categorize_relationships(
        dbt_tag, _DBT_TAG_REL_FIELDS, DbtTagRelationshipAttributes
    )
    return DbtTagNested(
        guid=dbt_tag.guid,
...
def _dbt_tag_from_nested(nested: DbtTagNested) -> DbtTag:
    ...
    kwargs.update(_extract_dbt_tag_attrs(attrs))
    kwargs.update(merged_rels)
    return DbtTag(**kwargs)
...
DbtTag.DBT_ALIAS = KeywordField("dbtAlias", "dbtAlias")
DbtTag.DBT_META = KeywordField("dbtMeta", "dbtMeta")
DbtTag.DBT_UNIQUE_ID = KeywordField("dbtUniqueId", "dbtUniqueId")
DbtTag.DBT_ACCOUNT_NAME = KeywordField("dbtAccountName", "dbtAccountName")
DbtTag.DBT_PROJECT_NAME = KeywordField("dbtProjectName", "dbtProjectName")
DbtTag.DBT_PACKAGE_NAME = KeywordField("dbtPackageName", "dbtPackageName")
...
DbtTag.CONTEXT_REPOSITORIES = RelationField("contextRepositories")
...
DbtTag.MODEL_IMPLEMENTED_ATTRIBUTES = RelationField("modelImplementedAttributes")
...
DbtTag.PARTIAL_CHILD_OBJECTS = RelationField("partialChildObjects")
DbtTag.INPUT_TO_PROCESSES = RelationField("inputToProcesses")
DbtTag.OUTPUT_FROM_PROCESSES = RelationField("outputFromProcesses")
DbtTag.USER_DEF_RELATIONSHIP_TO = RelationField("userDefRelationshipTo")
DbtTag.USER_DEF_RELATIONSHIP_FROM = RelationField("userDefRelationshipFrom")
DbtTag.FILES = RelationField("files")
DbtTag.LINKS = RelationField("links")
DbtTag.README = RelationField("readme")
DbtTag.SCHEMA_REGISTRY_SUBJECTS = RelationField("schemaRegistrySubjects")
DbtTag.SODA_CHECKS = RelationField("sodaChecks")
DbtTag.INPUT_TO_SPARK_JOBS = RelationField("inputToSparkJobs")
DbtTag.OUTPUT_FROM_SPARK_JOBS = RelationField("outputFromSparkJobs")
