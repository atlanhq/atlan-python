...
class DatabricksDashboard(Asset):
    ...
    def validate(self, for_creation: bool = False) -> None:
        """
        Dry-run validation of this DatabricksDashboard instance.
        ...
        if errors:
            raise ValueError(f"DatabricksDashboard validation failed: {errors}")

    def minimize(self) -> "DatabricksDashboard":
        """
        Return a minimal copy of this DatabricksDashboard with only updater-required fields.

        Calls :meth:`validate` first to ensure the instance is valid, then
        returns a new DatabricksDashboard with only the fields needed for an update
        (qualified_name, name, and any type-specific additional fields).

        Returns:
            A new DatabricksDashboard instance with only the minimum required fields.
        """
        self.validate()
        return DatabricksDashboard(qualified_name=self.qualified_name, name=self.name)

    def relate(self) -> "RelatedDatabricksDashboard":
        """
        Create a :class:`RelatedDatabricksDashboard` reference from this instance.
        ...
        Returns:
            A RelatedDatabricksDashboard reference to this asset.
        """
        if self.guid is not UNSET:
            return RelatedDatabricksDashboard(guid=self.guid)
        return RelatedDatabricksDashboard(qualified_name=self.qualified_name)
...
class DatabricksDashboardNested(AssetNested):
    ...
    relationship_attributes: Union[
        DatabricksDashboardRelationshipAttributes, UnsetType
    ] = UNSET
...
def _populate_databricks_dashboard_attrs(
    attrs: DatabricksDashboardAttributes, obj: DatabricksDashboard
) -> None:
...
def _databricks_dashboard_to_nested(
    databricks_dashboard: DatabricksDashboard,
) -> DatabricksDashboardNested:
    ...
    return DatabricksDashboardNested(
        ...
        depth=databricks_dashboard.depth,
        immediate_upstream=databricks_dashboard.immediate_upstream,
        immediate_downstream=databricks_dashboard.immediate_downstream,
        attributes=attrs,
...
def _databricks_dashboard_from_nested(
    nested: DatabricksDashboardNested,
) -> DatabricksDashboard:
    """Convert nested format to flat DatabricksDashboard."""
    attrs = (
        nested.attributes
        if nested.attributes is not UNSET
        else DatabricksDashboardAttributes()
    )
    ...
    kwargs.update(_extract_databricks_dashboard_attrs(attrs))
    kwargs.update(merged_rels)
    return DatabricksDashboard(**kwargs)


def _databricks_dashboard_to_nested_bytes(
    databricks_dashboard: DatabricksDashboard, serde: Serde
) -> bytes:
...
def _databricks_dashboard_from_nested_bytes(
...
DatabricksDashboard.QUERY_COUNT_UPDATED_AT = NumericField(
    "queryCountUpdatedAt", "queryCountUpdatedAt"
)
DatabricksDashboard.DATABASE_NAME = KeywordField("databaseName", "databaseName")
DatabricksDashboard.DATABASE_QUALIFIED_NAME = KeywordField(
    "databaseQualifiedName", "databaseQualifiedName"
)
DatabricksDashboard.SCHEMA_NAME = KeywordField("schemaName", "schemaName")
DatabricksDashboard.SCHEMA_QUALIFIED_NAME = KeywordField(
    "schemaQualifiedName", "schemaQualifiedName"
)
DatabricksDashboard.TABLE_NAME = KeywordField("tableName", "tableName")
DatabricksDashboard.TABLE_QUALIFIED_NAME = KeywordField(
    "tableQualifiedName", "tableQualifiedName"
)
DatabricksDashboard.VIEW_NAME = KeywordField("viewName", "viewName")
DatabricksDashboard.VIEW_QUALIFIED_NAME = KeywordField(
    "viewQualifiedName", "viewQualifiedName"
)
DatabricksDashboard.CALCULATION_VIEW_NAME = KeywordField(
    "calculationViewName", "calculationViewName"
)
DatabricksDashboard.CALCULATION_VIEW_QUALIFIED_NAME = KeywordField(
...
DatabricksDashboard.SQL_AI_MODEL_CONTEXT_QUALIFIED_NAME = KeywordField(
    "sqlAIModelContextQualifiedName", "sqlAIModelContextQualifiedName"
)
DatabricksDashboard.SQL_IS_SECURE = BooleanField("sqlIsSecure", "sqlIsSecure")
DatabricksDashboard.SQL_HAS_AI_INSIGHTS = BooleanField(
    "sqlHasAiInsights", "sqlHasAiInsights"
)
DatabricksDashboard.SQL_AI_INSIGHTS_LAST_ANALYZED_AT = NumericField(
    "sqlAiInsightsLastAnalyzedAt", "sqlAiInsightsLastAnalyzedAt"
)
DatabricksDashboard.SQL_AI_INSIGHTS_POPULAR_BUSINESS_QUESTION_COUNT = NumericField(
    "sqlAiInsightsPopularBusinessQuestionCount",
    "sqlAiInsightsPopularBusinessQuestionCount",
)
DatabricksDashboard.SQL_AI_INSIGHTS_POPULAR_JOIN_COUNT = NumericField(
    "sqlAiInsightsPopularJoinCount", "sqlAiInsightsPopularJoinCount"
)
DatabricksDashboard.SQL_AI_INSIGHTS_POPULAR_FILTER_COUNT = NumericField(
    "sqlAiInsightsPopularFilterCount", "sqlAiInsightsPopularFilterCount"
)
DatabricksDashboard.SQL_AI_INSIGHTS_RELATIONSHIP_COUNT = NumericField(
    "sqlAiInsightsRelationshipCount", "sqlAiInsightsRelationshipCount"
)
DatabricksDashboard.SQL_COALESCE_LAST_RUN_STATUS = KeywordField(
    "sqlCoalesceLastRunStatus", "sqlCoalesceLastRunStatus"
)
DatabricksDashboard.SQL_COALESCE_NODE_STATUS = KeywordField(
    "sqlCoalesceNodeStatus", "sqlCoalesceNodeStatus"
)
DatabricksDashboard.SQL_COALESCE_LAST_RUN_AT = NumericField(
    "sqlCoalesceLastRunAt", "sqlCoalesceLastRunAt"
)
DatabricksDashboard.SQL_COALESCE_NODE_TYPE = KeywordField(
    "sqlCoalesceNodeType", "sqlCoalesceNodeType"
)
DatabricksDashboard.SQL_COALESCE_ENVIRONMENT_ID = KeywordField(
    "sqlCoalesceEnvironmentId", "sqlCoalesceEnvironmentId"
)
DatabricksDashboard.SQL_COALESCE_ENVIRONMENT_NAME = KeywordTextField(
    "sqlCoalesceEnvironmentName",
    "sqlCoalesceEnvironmentName",
    "sqlCoalesceEnvironmentName.text",
)
DatabricksDashboard.SQL_COALESCE_PROJECT_ID = KeywordField(
    "sqlCoalesceProjectId", "sqlCoalesceProjectId"
)
DatabricksDashboard.SQL_COALESCE_PROJECT_NAME = KeywordTextField(
    "sqlCoalesceProjectName", "sqlCoalesceProjectName", "sqlCoalesceProjectName.text"
)
DatabricksDashboard.SQL_SHARE_QUALIFIED_NAMES = KeywordField(
    "sqlShareQualifiedNames", "sqlShareQualifiedNames"
)
DatabricksDashboard.CATALOG_DATASET_GUID = KeywordField(
...
DatabricksDashboard.DATA_CONTRACT_LATEST_CERTIFIED = RelationField(
...
DatabricksDashboard.MODEL_IMPLEMENTED_ENTITIES = RelationField(
    "modelImplementedEntities"
)
DatabricksDashboard.MODEL_IMPLEMENTED_ATTRIBUTES = RelationField(
...
DatabricksDashboard.DQ_REFERENCE_DATASET_RULES = RelationField(
...
DatabricksDashboard.GCP_DATAPLEX_ASPECT_TYPE_METADATA_ENTITIES = RelationField(
...
DatabricksDashboard.USER_DEF_RELATIONSHIP_FROM = RelationField(
...
DatabricksDashboard.SNOWFLAKE_SEMANTIC_LOGICAL_TABLES = RelationField(
...
DatabricksDashboard.SQL_INSIGHT_OUTGOING_JOINS = RelationField(
    "sqlInsightOutgoingJoins"
)
DatabricksDashboard.SQL_INSIGHT_INCOMING_JOINS = RelationField(
    "sqlInsightIncomingJoins"
)
DatabricksDashboard.SQL_INSIGHT_BUSINESS_QUESTIONS = RelationField(
