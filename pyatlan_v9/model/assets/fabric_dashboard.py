...
class FabricDashboard(Asset):
    ...
    def validate(self, for_creation: bool = False) -> None:
        """
        Dry-run validation of this FabricDashboard instance.
        ...
        if errors:
            raise ValueError(f"FabricDashboard validation failed: {errors}")

    def minimize(self) -> "FabricDashboard":
        """
        Return a minimal copy of this FabricDashboard with only updater-required fields.

        Calls :meth:`validate` first to ensure the instance is valid, then
        returns a new FabricDashboard with only the fields needed for an update
        (qualified_name, name, and any type-specific additional fields).

        Returns:
            A new FabricDashboard instance with only the minimum required fields.
        """
        self.validate()
        return FabricDashboard(qualified_name=self.qualified_name, name=self.name)

    def relate(self) -> "RelatedFabricDashboard":
        """
        Create a :class:`RelatedFabricDashboard` reference from this instance.
        ...
        Returns:
            A RelatedFabricDashboard reference to this asset.
        """
        if self.guid is not UNSET:
            return RelatedFabricDashboard(guid=self.guid)
        return RelatedFabricDashboard(qualified_name=self.qualified_name)
...
def _fabric_dashboard_to_nested(
    ...
    return FabricDashboardNested(
        ...
        depth=fabric_dashboard.depth,
        immediate_upstream=fabric_dashboard.immediate_upstream,
        immediate_downstream=fabric_dashboard.immediate_downstream,
        attributes=attrs,
...
def _fabric_dashboard_from_nested(nested: FabricDashboardNested) -> FabricDashboard:
    ...
    kwargs.update(_extract_fabric_dashboard_attrs(attrs))
    kwargs.update(merged_rels)
    return FabricDashboard(**kwargs)
...
FabricDashboard.CONTEXT_REPOSITORIES = RelationField("contextRepositories")
...
FabricDashboard.GCP_DATAPLEX_ASPECT_TYPE_METADATA_ENTITIES = RelationField(
    "gcpDataplexAspectTypeMetadataEntities"
)
FabricDashboard.MEANINGS = RelationField("meanings")
FabricDashboard.KNOWLEDGE_LINKED_FILES = RelationField("knowledgeLinkedFiles")
