...
from .sage_maker_related import RelatedSageMaker
...
class SageMaker(Asset):
    ...
    def validate(self, for_creation: bool = False) -> None:
        """
        Dry-run validation of this SageMaker instance.
        ...
        if errors:
            raise ValueError(f"SageMaker validation failed: {errors}")

    def minimize(self) -> "SageMaker":
        """
        Return a minimal copy of this SageMaker with only updater-required fields.

        Calls :meth:`validate` first to ensure the instance is valid, then
        returns a new SageMaker with only the fields needed for an update
        (qualified_name, name, and any type-specific additional fields).

        Returns:
            A new SageMaker instance with only the minimum required fields.
        """
        self.validate()
        return SageMaker(qualified_name=self.qualified_name, name=self.name)

    def relate(self) -> "RelatedSageMaker":
        """
        Create a :class:`RelatedSageMaker` reference from this instance.
        ...
        Returns:
            A RelatedSageMaker reference to this asset.
        """
        if self.guid is not UNSET:
            return RelatedSageMaker(guid=self.guid)
        return RelatedSageMaker(qualified_name=self.qualified_name)
...
def _sage_maker_from_nested(nested: SageMakerNested) -> SageMaker:
    ...
    kwargs.update(_extract_sage_maker_attrs(attrs))
    kwargs.update(merged_rels)
    return SageMaker(**kwargs)
...
SageMaker.CONTEXT_REPOSITORIES = RelationField("contextRepositories")
...
SageMaker.GCP_DATAPLEX_ASPECT_TYPE_METADATA_ENTITIES = RelationField(
