...
class CognosDashboard(Asset):
    ...
    def validate(self, for_creation: bool = False) -> None:
        """
        Dry-run validation of this CognosDashboard instance.
        ...
        if errors:
            raise ValueError(f"CognosDashboard validation failed: {errors}")

    def minimize(self) -> "CognosDashboard":
        """
        Return a minimal copy of this CognosDashboard with only updater-required fields.

        Calls :meth:`validate` first to ensure the instance is valid, then
        returns a new CognosDashboard with only the fields needed for an update
        (qualified_name, name, and any type-specific additional fields).

        Returns:
            A new CognosDashboard instance with only the minimum required fields.
        """
        self.validate()
        return CognosDashboard(qualified_name=self.qualified_name, name=self.name)

    def relate(self) -> "RelatedCognosDashboard":
        """
        Create a :class:`RelatedCognosDashboard` reference from this instance.
        ...
        Returns:
            A RelatedCognosDashboard reference to this asset.
        """
        if self.guid is not UNSET:
            return RelatedCognosDashboard(guid=self.guid)
        return RelatedCognosDashboard(qualified_name=self.qualified_name)
...
def _cognos_dashboard_from_nested(nested: CognosDashboardNested) -> CognosDashboard:
    ...
    kwargs.update(_extract_cognos_dashboard_attrs(attrs))
    kwargs.update(merged_rels)
    return CognosDashboard(**kwargs)
...
CognosDashboard.CATALOG_DATASET_GUID = KeywordField(
...
CognosDashboard.GCP_DATAPLEX_ASPECT_TYPE_METADATA_ENTITIES = RelationField(
    "gcpDataplexAspectTypeMetadataEntities"
)
CognosDashboard.MEANINGS = RelationField("meanings")
CognosDashboard.KNOWLEDGE_LINKED_FILES = RelationField("knowledgeLinkedFiles")
