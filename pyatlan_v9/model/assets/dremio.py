...
from .dremio_related import RelatedDremio
...
class Dremio(Asset):
    ...
    def validate(self, for_creation: bool = False) -> None:
        """
        Dry-run validation of this Dremio instance.
        ...
        if errors:
            raise ValueError(f"Dremio validation failed: {errors}")

    def minimize(self) -> "Dremio":
        """
        Return a minimal copy of this Dremio with only updater-required fields.

        Calls :meth:`validate` first to ensure the instance is valid, then
        returns a new Dremio with only the fields needed for an update
        (qualified_name, name, and any type-specific additional fields).

        Returns:
            A new Dremio instance with only the minimum required fields.
        """
        self.validate()
        return Dremio(qualified_name=self.qualified_name, name=self.name)

    def relate(self) -> "RelatedDremio":
        """
        Create a :class:`RelatedDremio` reference from this instance.
        ...
        Returns:
            A RelatedDremio reference to this asset.
        """
        if self.guid is not UNSET:
            return RelatedDremio(guid=self.guid)
        return RelatedDremio(qualified_name=self.qualified_name)
...
Dremio.SQL_HAS_AI_INSIGHTS = BooleanField("sqlHasAiInsights", "sqlHasAiInsights")
Dremio.SQL_AI_INSIGHTS_LAST_ANALYZED_AT = NumericField(
    "sqlAiInsightsLastAnalyzedAt", "sqlAiInsightsLastAnalyzedAt"
)
Dremio.SQL_AI_INSIGHTS_POPULAR_BUSINESS_QUESTION_COUNT = NumericField(
    "sqlAiInsightsPopularBusinessQuestionCount",
    "sqlAiInsightsPopularBusinessQuestionCount",
)
Dremio.SQL_AI_INSIGHTS_POPULAR_JOIN_COUNT = NumericField(
    "sqlAiInsightsPopularJoinCount", "sqlAiInsightsPopularJoinCount"
)
Dremio.SQL_AI_INSIGHTS_POPULAR_FILTER_COUNT = NumericField(
    "sqlAiInsightsPopularFilterCount", "sqlAiInsightsPopularFilterCount"
)
Dremio.SQL_AI_INSIGHTS_RELATIONSHIP_COUNT = NumericField(
    "sqlAiInsightsRelationshipCount", "sqlAiInsightsRelationshipCount"
)
Dremio.SQL_COALESCE_LAST_RUN_STATUS = KeywordField(
    "sqlCoalesceLastRunStatus", "sqlCoalesceLastRunStatus"
)
Dremio.SQL_COALESCE_NODE_STATUS = KeywordField(
    "sqlCoalesceNodeStatus", "sqlCoalesceNodeStatus"
)
Dremio.SQL_COALESCE_LAST_RUN_AT = NumericField(
    "sqlCoalesceLastRunAt", "sqlCoalesceLastRunAt"
)
Dremio.SQL_COALESCE_NODE_TYPE = KeywordField(
    "sqlCoalesceNodeType", "sqlCoalesceNodeType"
)
Dremio.SQL_COALESCE_ENVIRONMENT_ID = KeywordField(
    "sqlCoalesceEnvironmentId", "sqlCoalesceEnvironmentId"
)
Dremio.SQL_COALESCE_ENVIRONMENT_NAME = KeywordTextField(
    "sqlCoalesceEnvironmentName",
    "sqlCoalesceEnvironmentName",
    "sqlCoalesceEnvironmentName.text",
)
Dremio.SQL_COALESCE_PROJECT_ID = KeywordField(
    "sqlCoalesceProjectId", "sqlCoalesceProjectId"
)
Dremio.SQL_COALESCE_PROJECT_NAME = KeywordTextField(
    "sqlCoalesceProjectName", "sqlCoalesceProjectName", "sqlCoalesceProjectName.text"
)
Dremio.SQL_SHARE_QUALIFIED_NAMES = KeywordField(
    "sqlShareQualifiedNames", "sqlShareQualifiedNames"
)
Dremio.CATALOG_DATASET_GUID = KeywordField("catalogDatasetGuid", "catalogDatasetGuid")
...
Dremio.CONTEXT_REPOSITORIES = RelationField("contextRepositories")
Dremio.DATA_CONTRACT_LATEST = RelationField("dataContractLatest")
Dremio.DATA_CONTRACT_LATEST_CERTIFIED = RelationField("dataContractLatestCertified")
...
Dremio.GCP_DATAPLEX_ASPECT_TYPE_METADATA_ENTITIES = RelationField(
    "gcpDataplexAspectTypeMetadataEntities"
)
Dremio.MEANINGS = RelationField("meanings")
Dremio.KNOWLEDGE_LINKED_FILES = RelationField("knowledgeLinkedFiles")
...
Dremio.SQL_INSIGHT_OUTGOING_JOINS = RelationField("sqlInsightOutgoingJoins")
Dremio.SQL_INSIGHT_INCOMING_JOINS = RelationField("sqlInsightIncomingJoins")
Dremio.SQL_INSIGHT_BUSINESS_QUESTIONS = RelationField("sqlInsightBusinessQuestions")
