...
class DataverseEntity(Asset):
    ...
    DATAVERSE_ENTITY_SCHEMA_NAME: ClassVar[Any] = None
    DATAVERSE_ENTITY_TABLE_TYPE: ClassVar[Any] = None
    ...
    def validate(self, for_creation: bool = False) -> None:
        """
        Dry-run validation of this DataverseEntity instance.
        ...
        if errors:
            raise ValueError(f"DataverseEntity validation failed: {errors}")

    def minimize(self) -> "DataverseEntity":
        """
        Return a minimal copy of this DataverseEntity with only updater-required fields.

        Calls :meth:`validate` first to ensure the instance is valid, then
        returns a new DataverseEntity with only the fields needed for an update
        (qualified_name, name, and any type-specific additional fields).

        Returns:
            A new DataverseEntity instance with only the minimum required fields.
        """
        self.validate()
        return DataverseEntity(qualified_name=self.qualified_name, name=self.name)

    def relate(self) -> "RelatedDataverseEntity":
        """
        Create a :class:`RelatedDataverseEntity` reference from this instance.
        ...
        Returns:
            A RelatedDataverseEntity reference to this asset.
        """
        if self.guid is not UNSET:
            return RelatedDataverseEntity(guid=self.guid)
        return RelatedDataverseEntity(qualified_name=self.qualified_name)
...
_DATAVERSE_ENTITY_REL_FIELDS: List[str] = [
    *_ASSET_REL_FIELDS,
...
def _populate_dataverse_entity_attrs(
    ...
    attrs.dataverse_entity_table_type = obj.dataverse_entity_table_type
...
def _extract_dataverse_entity_attrs(attrs: DataverseEntityAttributes) -> dict:
    ...
    result["dataverse_entity_schema_name"] = attrs.dataverse_entity_schema_name
    result["dataverse_entity_table_type"] = attrs.dataverse_entity_table_type
...
def _dataverse_entity_from_nested(nested: DataverseEntityNested) -> DataverseEntity:
    ...
    kwargs.update(_extract_dataverse_entity_attrs(attrs))
...
DataverseEntity.CONTEXT_REPOSITORIES = RelationField("contextRepositories")
...
DataverseEntity.PARTIAL_CHILD_OBJECTS = RelationField("partialChildObjects")
DataverseEntity.INPUT_TO_PROCESSES = RelationField("inputToProcesses")
DataverseEntity.OUTPUT_FROM_PROCESSES = RelationField("outputFromProcesses")
DataverseEntity.USER_DEF_RELATIONSHIP_TO = RelationField("userDefRelationshipTo")
DataverseEntity.USER_DEF_RELATIONSHIP_FROM = RelationField("userDefRelationshipFrom")
DataverseEntity.FILES = RelationField("files")
DataverseEntity.LINKS = RelationField("links")
DataverseEntity.README = RelationField("readme")
DataverseEntity.SCHEMA_REGISTRY_SUBJECTS = RelationField("schemaRegistrySubjects")
DataverseEntity.SODA_CHECKS = RelationField("sodaChecks")
DataverseEntity.INPUT_TO_SPARK_JOBS = RelationField("inputToSparkJobs")
DataverseEntity.OUTPUT_FROM_SPARK_JOBS = RelationField("outputFromSparkJobs")
