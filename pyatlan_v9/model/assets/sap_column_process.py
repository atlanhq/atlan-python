...
class SAPColumnProcessNested(AssetNested):
    ...
    relationship_attributes: Union[
        SAPColumnProcessRelationshipAttributes, UnsetType
    ] = UNSET
...
def _populate_sap_column_process_attrs(
    attrs: SAPColumnProcessAttributes, obj: SAPColumnProcess
) -> None:
...
def _sap_column_process_to_nested(
    sap_column_process: SAPColumnProcess,
) -> SAPColumnProcessNested:
...
def _sap_column_process_from_nested(nested: SAPColumnProcessNested) -> SAPColumnProcess:
    ...
    attrs = (
        ...
        else SAPColumnProcessAttributes()
    )
    ...
    kwargs.update(_extract_sap_column_process_attrs(attrs))
    kwargs.update(merged_rels)
    return SAPColumnProcess(**kwargs)


def _sap_column_process_to_nested_bytes(
    sap_column_process: SAPColumnProcess, serde: Serde
) -> bytes:
    """Convert flat SAPColumnProcess to nested JSON bytes."""
    return serde.encode(_sap_column_process_to_nested(sap_column_process))


def _sap_column_process_from_nested_bytes(
...
SAPColumnProcess.SAP_TECHNICAL_NAME = KeywordField(
...
SAPColumnProcess.SAP_COMPONENT_NAME = KeywordField(
...
SAPColumnProcess.BIGQUERY_ROUTINES = RelationField("bigqueryRoutines")
...
SAPColumnProcess.DATA_CONTRACT_LATEST_CERTIFIED = RelationField(
...
SAPColumnProcess.MODEL_IMPLEMENTED_ATTRIBUTES = RelationField(
...
SAPColumnProcess.GCP_DATAPLEX_ASPECT_TYPE_METADATA_ENTITIES = RelationField(
