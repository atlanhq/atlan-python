...
class MicroStrategyDocument(Asset):
    ...
    def validate(self, for_creation: bool = False) -> None:
        """
        Dry-run validation of this MicroStrategyDocument instance.
        ...
        if errors:
            raise ValueError(f"MicroStrategyDocument validation failed: {errors}")

    def minimize(self) -> "MicroStrategyDocument":
        """
        Return a minimal copy of this MicroStrategyDocument with only updater-required fields.

        Calls :meth:`validate` first to ensure the instance is valid, then
        returns a new MicroStrategyDocument with only the fields needed for an update
        (qualified_name, name, and any type-specific additional fields).

        Returns:
            A new MicroStrategyDocument instance with only the minimum required fields.
        """
        self.validate()
        return MicroStrategyDocument(qualified_name=self.qualified_name, name=self.name)

    def relate(self) -> "RelatedMicroStrategyDocument":
        """
        Create a :class:`RelatedMicroStrategyDocument` reference from this instance.
        ...
        Returns:
            A RelatedMicroStrategyDocument reference to this asset.
        """
        if self.guid is not UNSET:
            return RelatedMicroStrategyDocument(guid=self.guid)
        return RelatedMicroStrategyDocument(qualified_name=self.qualified_name)
...
def _micro_strategy_document_to_nested(
    ...
    return MicroStrategyDocumentNested(
        ...
        depth=micro_strategy_document.depth,
        immediate_upstream=micro_strategy_document.immediate_upstream,
        immediate_downstream=micro_strategy_document.immediate_downstream,
        attributes=attrs,
...
MicroStrategyDocument.CONTEXT_REPOSITORIES = RelationField("contextRepositories")
...
MicroStrategyDocument.KNOWLEDGE_LINKED_FILES = RelationField("knowledgeLinkedFiles")
