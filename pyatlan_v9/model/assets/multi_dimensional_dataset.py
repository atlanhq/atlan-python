...
class MultiDimensionalDataset(Asset):
    ...
    def validate(self, for_creation: bool = False) -> None:
        """
        Dry-run validation of this MultiDimensionalDataset instance.
        ...
        if errors:
            raise ValueError(f"MultiDimensionalDataset validation failed: {errors}")

    def minimize(self) -> "MultiDimensionalDataset":
        """
        Return a minimal copy of this MultiDimensionalDataset with only updater-required fields.

        Calls :meth:`validate` first to ensure the instance is valid, then
        returns a new MultiDimensionalDataset with only the fields needed for an update
        (qualified_name, name, and any type-specific additional fields).

        Returns:
            A new MultiDimensionalDataset instance with only the minimum required fields.
        """
        self.validate()
    ...
    def relate(self) -> "RelatedMultiDimensionalDataset":
        """
        Create a :class:`RelatedMultiDimensionalDataset` reference from this instance.
        ...
        Returns:
            A RelatedMultiDimensionalDataset reference to this asset.
        """
        if self.guid is not UNSET:
            return RelatedMultiDimensionalDataset(guid=self.guid)
        return RelatedMultiDimensionalDataset(qualified_name=self.qualified_name)
...
def _multi_dimensional_dataset_from_nested(
    ...
    kwargs.update(_extract_multi_dimensional_dataset_attrs(attrs))
    kwargs.update(merged_rels)
    return MultiDimensionalDataset(**kwargs)
