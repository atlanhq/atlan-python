...
class DocumentDBDatabase(Asset):
    ...
    DOCUMENT_DB_DATABASE_COLLECTION_COUNT: ClassVar[Any] = None
    ...
    DOCUMENT_DB_COLLECTIONS: ClassVar[Any] = None
    ...
    def validate(self, for_creation: bool = False) -> None:
        """
        Dry-run validation of this DocumentDBDatabase instance.
        ...
        if errors:
            raise ValueError(f"DocumentDBDatabase validation failed: {errors}")

    def minimize(self) -> "DocumentDBDatabase":
        """
        Return a minimal copy of this DocumentDBDatabase with only updater-required fields.

        Calls :meth:`validate` first to ensure the instance is valid, then
        returns a new DocumentDBDatabase with only the fields needed for an update
        (qualified_name, name, and any type-specific additional fields).

        Returns:
            A new DocumentDBDatabase instance with only the minimum required fields.
        """
        self.validate()
        return DocumentDBDatabase(qualified_name=self.qualified_name, name=self.name)

    def relate(self) -> "RelatedDocumentDBDatabase":
        """
        Create a :class:`RelatedDocumentDBDatabase` reference from this instance.
        ...
        Returns:
            A RelatedDocumentDBDatabase reference to this asset.
        """
        if self.guid is not UNSET:
            return RelatedDocumentDBDatabase(guid=self.guid)
        return RelatedDocumentDBDatabase(qualified_name=self.qualified_name)
...
_DOCUMENT_DB_DATABASE_REL_FIELDS: List[str] = [
    *_ASSET_REL_FIELDS,
...
def _document_db_database_to_nested(
    ...
    return DocumentDBDatabaseNested(
        ...
        depth=document_db_database.depth,
        immediate_upstream=document_db_database.immediate_upstream,
        immediate_downstream=document_db_database.immediate_downstream,
        attributes=attrs,
...
def _document_db_database_from_nested(
    ...
    kwargs.update(_extract_document_db_database_attrs(attrs))
    kwargs.update(merged_rels)
    return DocumentDBDatabase(**kwargs)
...
DocumentDBDatabase.SQL_IS_IMPORTED_VIA_DATA_SHARE = BooleanField(
    "sqlIsImportedViaDataShare", "sqlIsImportedViaDataShare"
)
DocumentDBDatabase.SQL_ORIGIN_ACCOUNT_GUID = KeywordField(
...
DocumentDBDatabase.SQL_COALESCE_LAST_RUN_STATUS = KeywordField(
    "sqlCoalesceLastRunStatus", "sqlCoalesceLastRunStatus"
)
DocumentDBDatabase.SQL_COALESCE_NODE_STATUS = KeywordField(
    "sqlCoalesceNodeStatus", "sqlCoalesceNodeStatus"
)
DocumentDBDatabase.SQL_COALESCE_LAST_RUN_AT = NumericField(
    "sqlCoalesceLastRunAt", "sqlCoalesceLastRunAt"
)
DocumentDBDatabase.SQL_COALESCE_NODE_TYPE = KeywordField(
    "sqlCoalesceNodeType", "sqlCoalesceNodeType"
)
DocumentDBDatabase.SQL_COALESCE_ENVIRONMENT_ID = KeywordField(
    "sqlCoalesceEnvironmentId", "sqlCoalesceEnvironmentId"
)
DocumentDBDatabase.SQL_COALESCE_ENVIRONMENT_NAME = KeywordTextField(
    "sqlCoalesceEnvironmentName",
    "sqlCoalesceEnvironmentName",
    "sqlCoalesceEnvironmentName.text",
)
DocumentDBDatabase.SQL_COALESCE_PROJECT_ID = KeywordField(
    "sqlCoalesceProjectId", "sqlCoalesceProjectId"
)
DocumentDBDatabase.SQL_COALESCE_PROJECT_NAME = KeywordTextField(
    "sqlCoalesceProjectName", "sqlCoalesceProjectName", "sqlCoalesceProjectName.text"
)
DocumentDBDatabase.SQL_SHARE_QUALIFIED_NAMES = KeywordField(
...
DocumentDBDatabase.CONTEXT_REPOSITORIES = RelationField("contextRepositories")
...
DocumentDBDatabase.DBT_MODELS = RelationField("dbtModels")
DocumentDBDatabase.SQL_DBT_MODELS = RelationField("sqlDbtModels")
DocumentDBDatabase.DBT_TESTS = RelationField("dbtTests")
DocumentDBDatabase.DBT_SOURCES = RelationField("dbtSources")
DocumentDBDatabase.SQL_DBT_SOURCES = RelationField("sqlDBTSources")
...
DocumentDBDatabase.KNOWLEDGE_LINKED_FILES = RelationField("knowledgeLinkedFiles")
DocumentDBDatabase.MC_MONITORS = RelationField("mcMonitors")
DocumentDBDatabase.MC_INCIDENTS = RelationField("mcIncidents")
DocumentDBDatabase.PARTIAL_CHILD_FIELDS = RelationField("partialChildFields")
DocumentDBDatabase.PARTIAL_CHILD_OBJECTS = RelationField("partialChildObjects")
DocumentDBDatabase.INPUT_TO_PROCESSES = RelationField("inputToProcesses")
DocumentDBDatabase.OUTPUT_FROM_PROCESSES = RelationField("outputFromProcesses")
DocumentDBDatabase.USER_DEF_RELATIONSHIP_TO = RelationField("userDefRelationshipTo")
DocumentDBDatabase.USER_DEF_RELATIONSHIP_FROM = RelationField("userDefRelationshipFrom")
DocumentDBDatabase.FILES = RelationField("files")
