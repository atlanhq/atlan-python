...
class FabricReport(Asset):
    ...
    def validate(self, for_creation: bool = False) -> None:
        """
        Dry-run validation of this FabricReport instance.
    ...
    def minimize(self) -> "FabricReport":
        """
        Return a minimal copy of this FabricReport with only updater-required fields.

        Calls :meth:`validate` first to ensure the instance is valid, then
        returns a new FabricReport with only the fields needed for an update
        (qualified_name, name, and any type-specific additional fields).

        Returns:
            A new FabricReport instance with only the minimum required fields.
        """
        self.validate()
    ...
    def relate(self) -> "RelatedFabricReport":
        """
        Create a :class:`RelatedFabricReport` reference from this instance.
        ...
        Returns:
            A RelatedFabricReport reference to this asset.
        """
    ...
    def to_nested_bytes(self, serde: Serde | None = None) -> bytes:
        ...
        if serde is None:
            serde = get_serde()
        return _fabric_report_to_nested_bytes(self, serde)

    @staticmethod
    def from_json(json_data: str | bytes, serde: Serde | None = None) -> FabricReport:
        """
...
_FABRIC_REPORT_REL_FIELDS: List[str] = [
    *_ASSET_REL_FIELDS,
...
def _fabric_report_to_nested(fabric_report: FabricReport) -> FabricReportNested:
    ...
    _populate_fabric_report_attrs(attrs, fabric_report)
    # Categorize relationships by save semantic (REPLACE, APPEND, REMOVE)
    replace_rels, append_rels, remove_rels = categorize_relationships(
        fabric_report, _FABRIC_REPORT_REL_FIELDS, FabricReportRelationshipAttributes
    )
...
def _fabric_report_from_nested(nested: FabricReportNested) -> FabricReport:
    ...
    kwargs.update(_extract_fabric_report_attrs(attrs))
    kwargs.update(merged_rels)
    return FabricReport(**kwargs)
...
FabricReport.CATALOG_DATASET_GUID = KeywordField(
    "catalogDatasetGuid", "catalogDatasetGuid"
)
FabricReport.INPUT_TO_AIRFLOW_TASKS = RelationField("inputToAirflowTasks")
FabricReport.OUTPUT_FROM_AIRFLOW_TASKS = RelationField("outputFromAirflowTasks")
FabricReport.ANOMALO_CHECKS = RelationField("anomaloChecks")
FabricReport.APPLICATION = RelationField("application")
FabricReport.APPLICATION_FIELD = RelationField("applicationField")
FabricReport.CONTEXT_REPOSITORIES = RelationField("contextRepositories")
FabricReport.DATA_CONTRACT_LATEST = RelationField("dataContractLatest")
FabricReport.DATA_CONTRACT_LATEST_CERTIFIED = RelationField(
    "dataContractLatestCertified"
)
FabricReport.OUTPUT_PORT_DATA_PRODUCTS = RelationField("outputPortDataProducts")
FabricReport.INPUT_PORT_DATA_PRODUCTS = RelationField("inputPortDataProducts")
FabricReport.MODEL_IMPLEMENTED_ENTITIES = RelationField("modelImplementedEntities")
FabricReport.MODEL_IMPLEMENTED_ATTRIBUTES = RelationField("modelImplementedAttributes")
FabricReport.METRICS = RelationField("metrics")
FabricReport.DQ_BASE_DATASET_RULES = RelationField("dqBaseDatasetRules")
FabricReport.DQ_REFERENCE_DATASET_RULES = RelationField("dqReferenceDatasetRules")
FabricReport.FABRIC_WORKSPACE = RelationField("fabricWorkspace")
FabricReport.FABRIC_PAGES = RelationField("fabricPages")
FabricReport.GCP_DATAPLEX_ASPECT_TYPE_METADATA_ENTITIES = RelationField(
...
FabricReport.PARTIAL_CHILD_OBJECTS = RelationField("partialChildObjects")
FabricReport.INPUT_TO_PROCESSES = RelationField("inputToProcesses")
FabricReport.OUTPUT_FROM_PROCESSES = RelationField("outputFromProcesses")
FabricReport.USER_DEF_RELATIONSHIP_TO = RelationField("userDefRelationshipTo")
FabricReport.USER_DEF_RELATIONSHIP_FROM = RelationField("userDefRelationshipFrom")
FabricReport.FILES = RelationField("files")
FabricReport.LINKS = RelationField("links")
FabricReport.README = RelationField("readme")
FabricReport.SCHEMA_REGISTRY_SUBJECTS = RelationField("schemaRegistrySubjects")
FabricReport.SODA_CHECKS = RelationField("sodaChecks")
FabricReport.INPUT_TO_SPARK_JOBS = RelationField("inputToSparkJobs")
FabricReport.OUTPUT_FROM_SPARK_JOBS = RelationField("outputFromSparkJobs")
