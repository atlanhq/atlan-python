...
class SalesforceDashboard(Asset):
    ...
    DASHBOARD_TYPE: ClassVar[Any] = None
    ...
    def validate(self, for_creation: bool = False) -> None:
        """
        Dry-run validation of this SalesforceDashboard instance.
        ...
        if errors:
            raise ValueError(f"SalesforceDashboard validation failed: {errors}")

    def minimize(self) -> "SalesforceDashboard":
        """
        Return a minimal copy of this SalesforceDashboard with only updater-required fields.

        Calls :meth:`validate` first to ensure the instance is valid, then
        returns a new SalesforceDashboard with only the fields needed for an update
        (qualified_name, name, and any type-specific additional fields).

        Returns:
            A new SalesforceDashboard instance with only the minimum required fields.
        """
        self.validate()
        return SalesforceDashboard(qualified_name=self.qualified_name, name=self.name)

    def relate(self) -> "RelatedSalesforceDashboard":
        """
        Create a :class:`RelatedSalesforceDashboard` reference from this instance.
        ...
        Returns:
            A RelatedSalesforceDashboard reference to this asset.
        """
        if self.guid is not UNSET:
            return RelatedSalesforceDashboard(guid=self.guid)
        return RelatedSalesforceDashboard(qualified_name=self.qualified_name)
...
_SALESFORCE_DASHBOARD_REL_FIELDS: List[str] = [
    *_ASSET_REL_FIELDS,
...
def _populate_salesforce_dashboard_attrs(
    ...
    attrs.dashboard_type = obj.dashboard_type
...
def _extract_salesforce_dashboard_attrs(attrs: SalesforceDashboardAttributes) -> dict:
    ...
    result["dashboard_type"] = attrs.dashboard_type
...
def _salesforce_dashboard_from_nested(
    ...
    kwargs.update(_extract_salesforce_dashboard_attrs(attrs))
    kwargs.update(merged_rels)
    return SalesforceDashboard(**kwargs)
...
SalesforceDashboard.CONTEXT_REPOSITORIES = RelationField("contextRepositories")
...
SalesforceDashboard.KNOWLEDGE_LINKED_FILES = RelationField("knowledgeLinkedFiles")
SalesforceDashboard.MC_MONITORS = RelationField("mcMonitors")
SalesforceDashboard.MC_INCIDENTS = RelationField("mcIncidents")
SalesforceDashboard.PARTIAL_CHILD_FIELDS = RelationField("partialChildFields")
SalesforceDashboard.PARTIAL_CHILD_OBJECTS = RelationField("partialChildObjects")
...
SalesforceDashboard.ORGANIZATION = RelationField("organization")
SalesforceDashboard.REPORTS = RelationField("reports")
SalesforceDashboard.SCHEMA_REGISTRY_SUBJECTS = RelationField("schemaRegistrySubjects")
SalesforceDashboard.SODA_CHECKS = RelationField("sodaChecks")
SalesforceDashboard.INPUT_TO_SPARK_JOBS = RelationField("inputToSparkJobs")
SalesforceDashboard.OUTPUT_FROM_SPARK_JOBS = RelationField("outputFromSparkJobs")
