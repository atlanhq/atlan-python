...
class PresetDashboard(Asset):
    ...
    def validate(self, for_creation: bool = False) -> None:
        """
        Dry-run validation of this PresetDashboard instance.
        ...
        if for_creation:
            ...
            if self.preset_workspace is UNSET:
                errors.append("preset_workspace is required for creation")
            if self.preset_workspace_qualified_name is UNSET:
                errors.append(
                    "preset_workspace_qualified_name is required for creation"
                )
        if errors:
            raise ValueError(f"PresetDashboard validation failed: {errors}")

    def minimize(self) -> "PresetDashboard":
        """
        Return a minimal copy of this PresetDashboard with only updater-required fields.

        Calls :meth:`validate` first to ensure the instance is valid, then
        returns a new PresetDashboard with only the fields needed for an update
        (qualified_name, name, and any type-specific additional fields).

        Returns:
            A new PresetDashboard instance with only the minimum required fields.
        """
        self.validate()
        return PresetDashboard(qualified_name=self.qualified_name, name=self.name)

    def relate(self) -> "RelatedPresetDashboard":
        """
        Create a :class:`RelatedPresetDashboard` reference from this instance.
        ...
        Returns:
            A RelatedPresetDashboard reference to this asset.
        """
        if self.guid is not UNSET:
            return RelatedPresetDashboard(guid=self.guid)
        return RelatedPresetDashboard(qualified_name=self.qualified_name)
...
def _preset_dashboard_to_nested(
    ...
    return PresetDashboardNested(
        ...
        depth=preset_dashboard.depth,
        immediate_upstream=preset_dashboard.immediate_upstream,
        immediate_downstream=preset_dashboard.immediate_downstream,
        attributes=attrs,
...
PresetDashboard.CATALOG_DATASET_GUID = KeywordField(
...
PresetDashboard.CONTEXT_REPOSITORIES = RelationField("contextRepositories")
PresetDashboard.DATA_CONTRACT_LATEST = RelationField("dataContractLatest")
PresetDashboard.DATA_CONTRACT_LATEST_CERTIFIED = RelationField(
...
PresetDashboard.GCP_DATAPLEX_ASPECT_TYPE_METADATA_ENTITIES = RelationField(
    "gcpDataplexAspectTypeMetadataEntities"
)
PresetDashboard.MEANINGS = RelationField("meanings")
PresetDashboard.KNOWLEDGE_LINKED_FILES = RelationField("knowledgeLinkedFiles")
