...
class SparkJob(Asset):
    ...
    def validate(self, for_creation: bool = False) -> None:
        """
        Dry-run validation of this SparkJob instance.
        ...
        if errors:
            raise ValueError(f"SparkJob validation failed: {errors}")

    def minimize(self) -> "SparkJob":
        """
        Return a minimal copy of this SparkJob with only updater-required fields.

        Calls :meth:`validate` first to ensure the instance is valid, then
        returns a new SparkJob with only the fields needed for an update
        (qualified_name, name, and any type-specific additional fields).

        Returns:
            A new SparkJob instance with only the minimum required fields.
        """
        self.validate()
        return SparkJob(qualified_name=self.qualified_name, name=self.name)

    def relate(self) -> "RelatedSparkJob":
        """
        Create a :class:`RelatedSparkJob` reference from this instance.
        ...
        Returns:
            A RelatedSparkJob reference to this asset.
        """
        if self.guid is not UNSET:
            return RelatedSparkJob(guid=self.guid)
        return RelatedSparkJob(qualified_name=self.qualified_name)
...
def _spark_job_from_nested(nested: SparkJobNested) -> SparkJob:
    ...
    kwargs.update(_extract_spark_job_attrs(attrs))
    kwargs.update(merged_rels)
    return SparkJob(**kwargs)
...
SparkJob.CATALOG_DATASET_GUID = KeywordField("catalogDatasetGuid", "catalogDatasetGuid")
...
SparkJob.CONTEXT_REPOSITORIES = RelationField("contextRepositories")
SparkJob.DATA_CONTRACT_LATEST = RelationField("dataContractLatest")
SparkJob.DATA_CONTRACT_LATEST_CERTIFIED = RelationField("dataContractLatestCertified")
...
SparkJob.GCP_DATAPLEX_ASPECT_TYPE_METADATA_ENTITIES = RelationField(
