...
"""
KnowledgeFolder asset model with flattened inheritance.

This module provides:
- KnowledgeFolder: Flat asset class (easy to use)
- KnowledgeFolderAttributes: Nested attributes struct (extends AssetAttributes)
- KnowledgeFolderNested: Nested API format struct
"""
...
class KnowledgeFolder(Asset):
    """
    Platform-managed container for unstructured context files in Atlan.
    """
    ...
    def from_json(
        ...
        Returns:
            KnowledgeFolder instance
        """
        if isinstance(json_data, str):
            json_data = json_data.encode("utf-8")
        if serde is None:
            serde = get_serde()
        return _knowledge_folder_from_nested_bytes(json_data, serde)
...
_KNOWLEDGE_FOLDER_REL_FIELDS: List[str] = [
    *_ASSET_REL_FIELDS,
...
def _knowledge_folder_to_nested(
    ...
    return KnowledgeFolderNested(
        ...
        type_name=knowledge_folder.type_name,
        status=knowledge_folder.status,
        version=knowledge_folder.version,
        create_time=knowledge_folder.create_time,
        update_time=knowledge_folder.update_time,
        created_by=knowledge_folder.created_by,
        updated_by=knowledge_folder.updated_by,
        classifications=knowledge_folder.classifications,
        classification_names=knowledge_folder.classification_names,
        meanings=knowledge_folder.meanings,
        labels=knowledge_folder.labels,
        business_attributes=knowledge_folder.business_attributes,
        custom_attributes=knowledge_folder.custom_attributes,
        pending_tasks=knowledge_folder.pending_tasks,
        proxy=knowledge_folder.proxy,
...
def _knowledge_folder_from_nested(nested: KnowledgeFolderNested) -> KnowledgeFolder:
    ...
    kwargs.update(_extract_knowledge_folder_attrs(attrs))
    kwargs.update(merged_rels)
    return KnowledgeFolder(**kwargs)
...
def _knowledge_folder_from_nested_bytes(data: bytes, serde: Serde) -> KnowledgeFolder:
    """Convert nested JSON bytes to flat KnowledgeFolder."""
...
KnowledgeFolder.PARTIAL_CHILD_OBJECTS = RelationField("partialChildObjects")
KnowledgeFolder.INPUT_TO_PROCESSES = RelationField("inputToProcesses")
KnowledgeFolder.OUTPUT_FROM_PROCESSES = RelationField("outputFromProcesses")
KnowledgeFolder.USER_DEF_RELATIONSHIP_TO = RelationField("userDefRelationshipTo")
KnowledgeFolder.USER_DEF_RELATIONSHIP_FROM = RelationField("userDefRelationshipFrom")
KnowledgeFolder.FILES = RelationField("files")
KnowledgeFolder.LINKS = RelationField("links")
KnowledgeFolder.README = RelationField("readme")
KnowledgeFolder.SCHEMA_REGISTRY_SUBJECTS = RelationField("schemaRegistrySubjects")
KnowledgeFolder.SODA_CHECKS = RelationField("sodaChecks")
KnowledgeFolder.INPUT_TO_SPARK_JOBS = RelationField("inputToSparkJobs")
KnowledgeFolder.OUTPUT_FROM_SPARK_JOBS = RelationField("outputFromSparkJobs")
