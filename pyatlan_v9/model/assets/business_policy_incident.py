...
"""
BusinessPolicyIncident asset model with flattened inheritance.

This module provides:
- BusinessPolicyIncident: Flat asset class (easy to use)
- BusinessPolicyIncidentAttributes: Nested attributes struct (extends AssetAttributes)
- BusinessPolicyIncidentNested: Nested API format struct
"""
...
class BusinessPolicyIncident(Asset):
    ...
    BUSINESS_POLICY_INCIDENT_NONCOMPLIANT_COUNT: ClassVar[Any] = None
    BUSINESS_POLICY_INCIDENT_RELATED_POLICY_GUIDS: ClassVar[Any] = None
    BUSINESS_POLICY_INCIDENT_FILTER_DSL: ClassVar[Any] = None
    ...
    def validate(self, for_creation: bool = False) -> None:
        """
        Dry-run validation of this BusinessPolicyIncident instance.
        ...
        if errors:
            raise ValueError(f"BusinessPolicyIncident validation failed: {errors}")

    def minimize(self) -> "BusinessPolicyIncident":
        """
        Return a minimal copy of this BusinessPolicyIncident with only updater-required fields.

        Calls :meth:`validate` first to ensure the instance is valid, then
        returns a new BusinessPolicyIncident with only the fields needed for an update
        (qualified_name, name, and any type-specific additional fields).

        Returns:
            A new BusinessPolicyIncident instance with only the minimum required fields.
        """
        self.validate()
    ...
    def relate(self) -> "RelatedBusinessPolicyIncident":
        """
        Create a :class:`RelatedBusinessPolicyIncident` reference from this instance.
        ...
        Returns:
            A RelatedBusinessPolicyIncident reference to this asset.
        """
        if self.guid is not UNSET:
            return RelatedBusinessPolicyIncident(guid=self.guid)
        return RelatedBusinessPolicyIncident(qualified_name=self.qualified_name)
    ...
    def from_json(
        ...
        Returns:
            BusinessPolicyIncident instance
        """
        if isinstance(json_data, str):
            json_data = json_data.encode("utf-8")
        if serde is None:
            serde = get_serde()
        return _business_policy_incident_from_nested_bytes(json_data, serde)
...
_BUSINESS_POLICY_INCIDENT_REL_FIELDS: List[str] = [
    *_ASSET_REL_FIELDS,
    "anomalo_checks",
    "application",
    "application_field",
...
def _business_policy_incident_to_nested(
    ...
    return BusinessPolicyIncidentNested(
        ...
        type_name=business_policy_incident.type_name,
        status=business_policy_incident.status,
        version=business_policy_incident.version,
        create_time=business_policy_incident.create_time,
        update_time=business_policy_incident.update_time,
        created_by=business_policy_incident.created_by,
        updated_by=business_policy_incident.updated_by,
        classifications=business_policy_incident.classifications,
        classification_names=business_policy_incident.classification_names,
        meanings=business_policy_incident.meanings,
        labels=business_policy_incident.labels,
        business_attributes=business_policy_incident.business_attributes,
        custom_attributes=business_policy_incident.custom_attributes,
        pending_tasks=business_policy_incident.pending_tasks,
        proxy=business_policy_incident.proxy,
        is_incomplete=business_policy_incident.is_incomplete,
        provenance_type=business_policy_incident.provenance_type,
        home_id=business_policy_incident.home_id,
        depth=business_policy_incident.depth,
        immediate_upstream=business_policy_incident.immediate_upstream,
        immediate_downstream=business_policy_incident.immediate_downstream,
        attributes=attrs,
...
def _business_policy_incident_from_nested(
    ...
    return BusinessPolicyIncident(
        ...
        **_extract_business_policy_incident_attrs(attrs),
...
BusinessPolicyIncident.CONTEXT_REPOSITORIES = RelationField("contextRepositories")
