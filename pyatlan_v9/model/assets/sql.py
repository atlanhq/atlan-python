...
class SQL(Asset):
    ...
    def validate(self, for_creation: bool = False) -> None:
        """
        Dry-run validation of this SQL instance.
    ...
    def minimize(self) -> "SQL":
        """
        Return a minimal copy of this SQL with only updater-required fields.

        Calls :meth:`validate` first to ensure the instance is valid, then
        returns a new SQL with only the fields needed for an update
        (qualified_name, name, and any type-specific additional fields).

        Returns:
            A new SQL instance with only the minimum required fields.
        """
        self.validate()
    ...
    def relate(self) -> "RelatedSQL":
        """
        Create a :class:`RelatedSQL` reference from this instance.
        ...
        Returns:
            A RelatedSQL reference to this asset.
        """
...
def _sql_to_nested(sql: SQL) -> SQLNested:
    ...
    return SQLNested(
        ...
        home_id=sql.home_id,
        depth=sql.depth,
        immediate_upstream=sql.immediate_upstream,
        immediate_downstream=sql.immediate_downstream,
        attributes=attrs,
...
def _sql_from_nested(nested: SQLNested) -> SQL:
    ...
    kwargs.update(_extract_sql_attrs(attrs))
...
SQL.SQL_COALESCE_LAST_RUN_STATUS = KeywordField(
    "sqlCoalesceLastRunStatus", "sqlCoalesceLastRunStatus"
)
SQL.SQL_COALESCE_NODE_STATUS = KeywordField(
    "sqlCoalesceNodeStatus", "sqlCoalesceNodeStatus"
)
SQL.SQL_COALESCE_LAST_RUN_AT = NumericField(
    "sqlCoalesceLastRunAt", "sqlCoalesceLastRunAt"
)
SQL.SQL_COALESCE_NODE_TYPE = KeywordField("sqlCoalesceNodeType", "sqlCoalesceNodeType")
SQL.SQL_COALESCE_ENVIRONMENT_ID = KeywordField(
    "sqlCoalesceEnvironmentId", "sqlCoalesceEnvironmentId"
)
SQL.SQL_COALESCE_ENVIRONMENT_NAME = KeywordTextField(
    "sqlCoalesceEnvironmentName",
    "sqlCoalesceEnvironmentName",
    "sqlCoalesceEnvironmentName.text",
)
SQL.SQL_COALESCE_PROJECT_ID = KeywordField(
    "sqlCoalesceProjectId", "sqlCoalesceProjectId"
)
SQL.SQL_COALESCE_PROJECT_NAME = KeywordTextField(
    "sqlCoalesceProjectName", "sqlCoalesceProjectName", "sqlCoalesceProjectName.text"
)
SQL.SQL_SHARE_QUALIFIED_NAMES = KeywordField(
...
SQL.CONTEXT_REPOSITORIES = RelationField("contextRepositories")
...
SQL.KNOWLEDGE_LINKED_FILES = RelationField("knowledgeLinkedFiles")
