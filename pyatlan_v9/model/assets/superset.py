...
from .superset_related import RelatedSuperset
...
class Superset(Asset):
    ...
    def validate(self, for_creation: bool = False) -> None:
        """
        Dry-run validation of this Superset instance.
        ...
        if errors:
            raise ValueError(f"Superset validation failed: {errors}")

    def minimize(self) -> "Superset":
        """
        Return a minimal copy of this Superset with only updater-required fields.

        Calls :meth:`validate` first to ensure the instance is valid, then
        returns a new Superset with only the fields needed for an update
        (qualified_name, name, and any type-specific additional fields).

        Returns:
            A new Superset instance with only the minimum required fields.
        """
        self.validate()
        return Superset(qualified_name=self.qualified_name, name=self.name)

    def relate(self) -> "RelatedSuperset":
        """
        Create a :class:`RelatedSuperset` reference from this instance.
        ...
        Returns:
            A RelatedSuperset reference to this asset.
        """
        if self.guid is not UNSET:
            return RelatedSuperset(guid=self.guid)
        return RelatedSuperset(qualified_name=self.qualified_name)
...
def _superset_to_nested(superset: Superset) -> SupersetNested:
    ...
    return SupersetNested(
        ...
        depth=superset.depth,
        immediate_upstream=superset.immediate_upstream,
        immediate_downstream=superset.immediate_downstream,
        attributes=attrs,
...
def _superset_from_nested(nested: SupersetNested) -> Superset:
    ...
    kwargs.update(_extract_superset_attrs(attrs))
...
Superset.CONTEXT_REPOSITORIES = RelationField("contextRepositories")
...
Superset.KNOWLEDGE_LINKED_FILES = RelationField("knowledgeLinkedFiles")
