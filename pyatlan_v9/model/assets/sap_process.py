...
"""
SAPProcess asset model with flattened inheritance.

This module provides:
- SAPProcess: Flat asset class (easy to use)
- SAPProcessAttributes: Nested attributes struct (extends AssetAttributes)
- SAPProcessNested: Nested API format struct
"""
...
@register_asset
class SAPProcess(Asset):
    """
    Asset-level lineage process for SAP. Subtype of Process used by all SAP connectors so SAP-specific lineage attributes stay scoped to the SAP namespace.
    """
    ...
    def to_nested_bytes(self, serde: Serde | None = None) -> bytes:
        ...
        if serde is None:
            serde = get_serde()
        return _sap_process_to_nested_bytes(self, serde)

    @staticmethod
    def from_json(json_data: str | bytes, serde: Serde | None = None) -> SAPProcess:
        """
        ...
        if serde is None:
            serde = get_serde()
        return _sap_process_from_nested_bytes(json_data, serde)
...
class SAPProcessAttributes(AssetAttributes):
    """SAPProcess-specific attributes for nested API format."""
...
class SAPProcessRelationshipAttributes(AssetRelationshipAttributes):
    """SAPProcess-specific relationship attributes for nested API format."""
...
class SAPProcessNested(AssetNested):
    """SAPProcess in nested API format for high-performance serialization."""

    attributes: Union[SAPProcessAttributes, UnsetType] = UNSET
    relationship_attributes: Union[SAPProcessRelationshipAttributes, UnsetType] = UNSET
    append_relationship_attributes: Union[SAPProcessRelationshipAttributes, UnsetType] = UNSET
    remove_relationship_attributes: Union[SAPProcessRelationshipAttributes, UnsetType] = UNSET

# =============================================================================
# CONVERSION HELPERS & CONSTANTS
# =============================================================================

_SAP_PROCESS_REL_FIELDS: List[str] = [
    *_ASSET_REL_FIELDS,
    "adf_activity",
    "airflow_tasks",
...
def _populate_sap_process_attrs(attrs: SAPProcessAttributes, obj: SAPProcess) -> None:
    """Populate SAPProcess-specific attributes on the attrs struct."""
...
def _extract_sap_process_attrs(attrs: SAPProcessAttributes) -> dict:
    """Extract all SAPProcess attributes from the attrs struct into a flat dict."""
...
def _sap_process_to_nested(sap_process: SAPProcess) -> SAPProcessNested:
    """Convert flat SAPProcess to nested format."""
    attrs = SAPProcessAttributes()
    _populate_sap_process_attrs(attrs, sap_process)
    # Categorize relationships by save semantic (REPLACE, APPEND, REMOVE)
    replace_rels, append_rels, remove_rels = categorize_relationships(
        sap_process, _SAP_PROCESS_REL_FIELDS, SAPProcessRelationshipAttributes
    )
    return SAPProcessNested(
        guid=sap_process.guid,
        type_name=sap_process.type_name,
        status=sap_process.status,
        version=sap_process.version,
        create_time=sap_process.create_time,
        update_time=sap_process.update_time,
        created_by=sap_process.created_by,
        updated_by=sap_process.updated_by,
        classifications=sap_process.classifications,
        classification_names=sap_process.classification_names,
        meanings=sap_process.meanings,
        labels=sap_process.labels,
        business_attributes=sap_process.business_attributes,
        custom_attributes=sap_process.custom_attributes,
        pending_tasks=sap_process.pending_tasks,
        proxy=sap_process.proxy,
...
def _sap_process_from_nested(nested: SAPProcessNested) -> SAPProcess:
    """Convert nested format to flat SAPProcess."""
    attrs = nested.attributes if nested.attributes is not UNSET else SAPProcessAttributes()
    # Merge relationships from all three buckets
    merged_rels = merge_relationships(
        nested.relationship_attributes,
        nested.append_relationship_attributes,
        nested.remove_relationship_attributes,
        _SAP_PROCESS_REL_FIELDS,
        SAPProcessRelationshipAttributes
    )
    return SAPProcess(
        guid=nested.guid,
...
def _sap_process_to_nested_bytes(sap_process: SAPProcess, serde: Serde) -> bytes:
    """Convert flat SAPProcess to nested JSON bytes."""
    return serde.encode(_sap_process_to_nested(sap_process))


def _sap_process_from_nested_bytes(data: bytes, serde: Serde) -> SAPProcess:
    """Convert nested JSON bytes to flat SAPProcess."""
    nested = serde.decode(data, SAPProcessNested)
    return _sap_process_from_nested(nested)
...
SAPProcess.SAP_TECHNICAL_NAME = KeywordField("sapTechnicalName", "sapTechnicalName")
SAPProcess.SAP_LOGICAL_NAME = KeywordField("sapLogicalName", "sapLogicalName")
SAPProcess.SAP_PACKAGE_NAME = KeywordField("sapPackageName", "sapPackageName")
SAPProcess.SAP_COMPONENT_NAME = KeywordField("sapComponentName", "sapComponentName")
SAPProcess.SAP_DATA_TYPE = KeywordField("sapDataType", "sapDataType")
SAPProcess.SAP_FIELD_COUNT = NumericField("sapFieldCount", "sapFieldCount")
SAPProcess.SAP_FIELD_ORDER = NumericField("sapFieldOrder", "sapFieldOrder")
SAPProcess.CATALOG_DATASET_GUID = KeywordField("catalogDatasetGuid", "catalogDatasetGuid")
SAPProcess.CODE = KeywordField("code", "code")
SAPProcess.SQL = KeywordField("sql", "sql")
SAPProcess.PARENT_CONNECTION_PROCESS_QUALIFIED_NAME = KeywordField("parentConnectionProcessQualifiedName", "parentConnectionProcessQualifiedName")
SAPProcess.AST = KeywordField("ast", "ast")
SAPProcess.ADDITIONAL_ETL_CONTEXT = KeywordField("additionalEtlContext", "additionalEtlContext")
...
SAPProcess.OUTPUT_FROM_AIRFLOW_TASKS = RelationField("outputFromAirflowTasks")
SAPProcess.ANOMALO_CHECKS = RelationField("anomaloChecks")
SAPProcess.APPLICATION = RelationField("application")
SAPProcess.APPLICATION_FIELD = RelationField("applicationField")
SAPProcess.CONTEXT_REPOSITORIES = RelationField("contextRepositories")
SAPProcess.DATA_CONTRACT_LATEST = RelationField("dataContractLatest")
SAPProcess.DATA_CONTRACT_LATEST_CERTIFIED = RelationField("dataContractLatestCertified")
SAPProcess.OUTPUT_PORT_DATA_PRODUCTS = RelationField("outputPortDataProducts")
SAPProcess.INPUT_PORT_DATA_PRODUCTS = RelationField("inputPortDataProducts")
SAPProcess.MODEL_IMPLEMENTED_ENTITIES = RelationField("modelImplementedEntities")
SAPProcess.MODEL_IMPLEMENTED_ATTRIBUTES = RelationField("modelImplementedAttributes")
SAPProcess.METRICS = RelationField("metrics")
SAPProcess.DQ_BASE_DATASET_RULES = RelationField("dqBaseDatasetRules")
SAPProcess.DQ_REFERENCE_DATASET_RULES = RelationField("dqReferenceDatasetRules")
SAPProcess.FABRIC_ACTIVITIES = RelationField("fabricActivities")
SAPProcess.FIVETRAN_CONNECTOR = RelationField("fivetranConnector")
SAPProcess.FLOW_ORCHESTRATED_BY = RelationField("flowOrchestratedBy")
SAPProcess.GCP_DATAPLEX_ASPECT_TYPE_METADATA_ENTITIES = RelationField("gcpDataplexAspectTypeMetadataEntities")
SAPProcess.MEANINGS = RelationField("meanings")
SAPProcess.MATILLION_COMPONENT = RelationField("matillionComponent")
SAPProcess.MC_MONITORS = RelationField("mcMonitors")
SAPProcess.MC_INCIDENTS = RelationField("mcIncidents")
SAPProcess.PARTIAL_CHILD_FIELDS = RelationField("partialChildFields")
SAPProcess.PARTIAL_CHILD_OBJECTS = RelationField("partialChildObjects")
SAPProcess.POWER_BI_DATAFLOW = RelationField("powerBIDataflow")
SAPProcess.INPUTS = RelationField("inputs")
SAPProcess.INPUT_TO_PROCESSES = RelationField("inputToProcesses")
SAPProcess.OUTPUTS = RelationField("outputs")
SAPProcess.OUTPUT_FROM_PROCESSES = RelationField("outputFromProcesses")
SAPProcess.COLUMN_PROCESSES = RelationField("columnProcesses")
SAPProcess.USER_DEF_RELATIONSHIP_TO = RelationField("userDefRelationshipTo")
SAPProcess.USER_DEF_RELATIONSHIP_FROM = RelationField("userDefRelationshipFrom")
SAPProcess.FILES = RelationField("files")
SAPProcess.LINKS = RelationField("links")
SAPProcess.README = RelationField("readme")
SAPProcess.SAP_BW_DTPS = RelationField("sapBwDtps")
SAPProcess.SQL_PROCEDURES = RelationField("sqlProcedures")
SAPProcess.SQL_FUNCTIONS = RelationField("sqlFunctions")
SAPProcess.SCHEMA_REGISTRY_SUBJECTS = RelationField("schemaRegistrySubjects")
SAPProcess.SODA_CHECKS = RelationField("sodaChecks")
SAPProcess.SPARK_JOBS = RelationField("sparkJobs")
SAPProcess.INPUT_TO_SPARK_JOBS = RelationField("inputToSparkJobs")
SAPProcess.OUTPUT_FROM_SPARK_JOBS = RelationField("outputFromSparkJobs")