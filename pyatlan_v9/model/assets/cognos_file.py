...
class CognosFile(Asset):
    ...
    def validate(self, for_creation: bool = False) -> None:
        """
        Dry-run validation of this CognosFile instance.
    ...
    def minimize(self) -> "CognosFile":
        """
        Return a minimal copy of this CognosFile with only updater-required fields.

        Calls :meth:`validate` first to ensure the instance is valid, then
        returns a new CognosFile with only the fields needed for an update
        (qualified_name, name, and any type-specific additional fields).

        Returns:
            A new CognosFile instance with only the minimum required fields.
        """
        self.validate()
    ...
    def relate(self) -> "RelatedCognosFile":
        """
        Create a :class:`RelatedCognosFile` reference from this instance.
        ...
        Returns:
            A RelatedCognosFile reference to this asset.
        """
    ...
    def to_nested_bytes(self, serde: Serde | None = None) -> bytes:
        ...
        if serde is None:
            serde = get_serde()
        return _cognos_file_to_nested_bytes(self, serde)

    @staticmethod
    def from_json(json_data: str | bytes, serde: Serde | None = None) -> CognosFile:
        """
...
_COGNOS_FILE_REL_FIELDS: List[str] = [
    *_ASSET_REL_FIELDS,
...
def _cognos_file_to_nested(cognos_file: CognosFile) -> CognosFileNested:
    ...
    _populate_cognos_file_attrs(attrs, cognos_file)
    # Categorize relationships by save semantic (REPLACE, APPEND, REMOVE)
    replace_rels, append_rels, remove_rels = categorize_relationships(
        cognos_file, _COGNOS_FILE_REL_FIELDS, CognosFileRelationshipAttributes
    )
    return CognosFileNested(
        ...
        depth=cognos_file.depth,
        immediate_upstream=cognos_file.immediate_upstream,
        immediate_downstream=cognos_file.immediate_downstream,
        attributes=attrs,
...
def _cognos_file_from_nested(nested: CognosFileNested) -> CognosFile:
    ...
    kwargs.update(_extract_cognos_file_attrs(attrs))
    kwargs.update(merged_rels)
    return CognosFile(**kwargs)
...
CognosFile.APPLICATION = RelationField("application")
...
CognosFile.MODEL_IMPLEMENTED_ATTRIBUTES = RelationField("modelImplementedAttributes")
CognosFile.METRICS = RelationField("metrics")
CognosFile.DQ_BASE_DATASET_RULES = RelationField("dqBaseDatasetRules")
CognosFile.DQ_REFERENCE_DATASET_RULES = RelationField("dqReferenceDatasetRules")
CognosFile.GCP_DATAPLEX_ASPECT_TYPE_METADATA_ENTITIES = RelationField(
    "gcpDataplexAspectTypeMetadataEntities"
)
CognosFile.MEANINGS = RelationField("meanings")
CognosFile.KNOWLEDGE_LINKED_FILES = RelationField("knowledgeLinkedFiles")
CognosFile.MC_MONITORS = RelationField("mcMonitors")
CognosFile.MC_INCIDENTS = RelationField("mcIncidents")
CognosFile.PARTIAL_CHILD_FIELDS = RelationField("partialChildFields")
CognosFile.PARTIAL_CHILD_OBJECTS = RelationField("partialChildObjects")
CognosFile.INPUT_TO_PROCESSES = RelationField("inputToProcesses")
CognosFile.OUTPUT_FROM_PROCESSES = RelationField("outputFromProcesses")
CognosFile.USER_DEF_RELATIONSHIP_TO = RelationField("userDefRelationshipTo")
CognosFile.USER_DEF_RELATIONSHIP_FROM = RelationField("userDefRelationshipFrom")
CognosFile.FILES = RelationField("files")
CognosFile.LINKS = RelationField("links")
CognosFile.README = RelationField("readme")
CognosFile.SCHEMA_REGISTRY_SUBJECTS = RelationField("schemaRegistrySubjects")
CognosFile.SODA_CHECKS = RelationField("sodaChecks")
CognosFile.INPUT_TO_SPARK_JOBS = RelationField("inputToSparkJobs")
CognosFile.OUTPUT_FROM_SPARK_JOBS = RelationField("outputFromSparkJobs")
