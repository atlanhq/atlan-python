...
class AI(Asset):
    ...
    def validate(self, for_creation: bool = False) -> None:
        """
        Dry-run validation of this AI instance.
        ...
        if errors:
            raise ValueError(f"AI validation failed: {errors}")

    def minimize(self) -> "AI":
        """
        Return a minimal copy of this AI with only updater-required fields.

        Calls :meth:`validate` first to ensure the instance is valid, then
        returns a new AI with only the fields needed for an update
        (qualified_name, name, and any type-specific additional fields).

        Returns:
            A new AI instance with only the minimum required fields.
        """
        self.validate()
        return AI(qualified_name=self.qualified_name, name=self.name)

    def relate(self) -> "RelatedAI":
        """
        Create a :class:`RelatedAI` reference from this instance.
        ...
        Returns:
            A RelatedAI reference to this asset.
        """
        if self.guid is not UNSET:
            return RelatedAI(guid=self.guid)
        return RelatedAI(qualified_name=self.qualified_name)
...
def _ai_from_nested(nested: AINested) -> AI:
    ...
    kwargs.update(_extract_ai_attrs(attrs))
...
AI.CATALOG_DATASET_GUID = KeywordField("catalogDatasetGuid", "catalogDatasetGuid")
...
AI.CONTEXT_REPOSITORIES = RelationField("contextRepositories")
...
AI.GCP_DATAPLEX_ASPECT_TYPE_METADATA_ENTITIES = RelationField(
    "gcpDataplexAspectTypeMetadataEntities"
)
AI.MEANINGS = RelationField("meanings")
AI.KNOWLEDGE_LINKED_FILES = RelationField("knowledgeLinkedFiles")
