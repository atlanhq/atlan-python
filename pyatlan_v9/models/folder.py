...
class Folder(Asset):
    ...
    # Override type_name with Folder-specific default
    ...
    def to_json(self, nested: bool = True, serde: Serde | None = None) -> str:
        ...
        if serde is None:
            serde = get_serde()
        if nested:
            return _folder_to_nested_bytes(self, serde).decode("utf-8")
        else:
            return serde.encode(self).decode("utf-8")

    @staticmethod
    def from_json(json_data: Union[str, bytes], serde: Serde | None = None) -> "Folder":
        """
...
def _folder_to_nested(folder: Folder) -> FolderNested:
    ...
    attrs = FolderAttributes(
        parent_qualified_name=folder.parent_qualified_name,
        collection_qualified_name=folder.collection_qualified_name,
    )
    # Categorize relationships by save semantic (REPLACE, APPEND, REMOVE)
    rel_fields: list[str] = ["children_folders", "parent"]
    replace_rels, append_rels, remove_rels = categorize_relationships(
        folder, rel_fields, FolderRelationshipAttributes
    )
    return FolderNested(
        guid=folder.guid,
