...
class App(Asset):
    ...
    # Override type_name with App-specific default
    ...
    def to_json(self, nested: bool = True, serde: Serde | None = None) -> str:
        ...
        if serde is None:
            serde = get_serde()
        if nested:
            return _app_to_nested_bytes(self, serde).decode("utf-8")
        else:
            return serde.encode(self).decode("utf-8")

    @staticmethod
    def from_json(json_data: Union[str, bytes], serde: Serde | None = None) -> "App":
        """
...
def _app_to_nested(app: App) -> AppNested:
    ...
    replace_rels, append_rels, remove_rels = categorize_relationships(
        app, rel_fields, AppRelationshipAttributes
    )
    return AppNested(
        guid=app.guid,
