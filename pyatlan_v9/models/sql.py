...
class SQL(Asset):
    ...
    # Override type_name with SQL-specific default
    ...
    def to_json(self, nested: bool = True, serde: Serde | None = None) -> str:
        ...
        if serde is None:
            serde = get_serde()
        if nested:
            return _sql_to_nested_bytes(self, serde).decode("utf-8")
        else:
            return serde.encode(self).decode("utf-8")

    @staticmethod
    def from_json(json_data: Union[str, bytes], serde: Serde | None = None) -> "SQL":
        """
...
def _sql_to_nested(sql: SQL) -> SQLNested:
    ...
    attrs = SQLAttributes(
        ...
        query_user_count=sql.query_user_count,
        query_user_map=sql.query_user_map,
        query_count_updated_at=sql.query_count_updated_at,
        database_name=sql.database_name,
        database_qualified_name=sql.database_qualified_name,
        schema_name=sql.schema_name,
        schema_qualified_name=sql.schema_qualified_name,
        table_name=sql.table_name,
        table_qualified_name=sql.table_qualified_name,
        view_name=sql.view_name,
        view_qualified_name=sql.view_qualified_name,
        calculation_view_name=sql.calculation_view_name,
        calculation_view_qualified_name=sql.calculation_view_qualified_name,
        is_profiled=sql.is_profiled,
        last_profiled_at=sql.last_profiled_at,
        sql_ai_model_context_qualified_name=sql.sql_ai_model_context_qualified_name,
        sql_is_secure=sql.sql_is_secure,
    )
    # Categorize relationships by save semantic (REPLACE, APPEND, REMOVE)
    rel_fields: list[str] = []
    replace_rels, append_rels, remove_rels = categorize_relationships(
        sql, rel_fields, SQLRelationshipAttributes
    )
    return SQLNested(
        guid=sql.guid,
