...
class FlowFieldOperation(Asset):
    ...
    # Override type_name with FlowFieldOperation-specific default
    ...
    def to_json(self, nested: bool = True, serde: Serde | None = None) -> str:
        ...
        if serde is None:
            serde = get_serde()
        if nested:
            return _flow_field_operation_to_nested_bytes(self, serde).decode("utf-8")
        else:
...
def _flow_field_operation_to_nested(
    ...
    attrs = FlowFieldOperationAttributes(
        flow_started_at=flow_field_operation.flow_started_at,
        flow_finished_at=flow_field_operation.flow_finished_at,
        flow_status=flow_field_operation.flow_status,
        flow_schedule=flow_field_operation.flow_schedule,
        flow_project_name=flow_field_operation.flow_project_name,
        flow_project_qualified_name=flow_field_operation.flow_project_qualified_name,
        flow_folder_name=flow_field_operation.flow_folder_name,
        flow_folder_qualified_name=flow_field_operation.flow_folder_qualified_name,
        flow_reusable_unit_name=flow_field_operation.flow_reusable_unit_name,
        flow_reusable_unit_qualified_name=flow_field_operation.flow_reusable_unit_qualified_name,
        flow_id=flow_field_operation.flow_id,
        flow_run_id=flow_field_operation.flow_run_id,
        flow_error_message=flow_field_operation.flow_error_message,
        flow_input_parameters=flow_field_operation.flow_input_parameters,
        code=flow_field_operation.code,
        sql=flow_field_operation.sql,
        parent_connection_process_qualified_name=flow_field_operation.parent_connection_process_qualified_name,
        ast=flow_field_operation.ast,
        additional_etl_context=flow_field_operation.additional_etl_context,
        ai_dataset_type=flow_field_operation.ai_dataset_type,
    )
    # Categorize relationships by save semantic (REPLACE, APPEND, REMOVE)
    rel_fields: list[str] = ["flow_orchestrated_by"]
    replace_rels, append_rels, remove_rels = categorize_relationships(
        flow_field_operation, rel_fields, FlowFieldOperationRelationshipAttributes
    )
