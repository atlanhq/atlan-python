...
class Azure(Asset):
    ...
    # Override type_name with Azure-specific default
    ...
    def to_json(self, nested: bool = True, serde: Serde | None = None) -> str:
        ...
        if serde is None:
            serde = get_serde()
        if nested:
            return _azure_to_nested_bytes(self, serde).decode("utf-8")
        else:
            return serde.encode(self).decode("utf-8")

    @staticmethod
    def from_json(json_data: Union[str, bytes], serde: Serde | None = None) -> "Azure":
        """
...
def _azure_to_nested(azure: Azure) -> AzureNested:
    ...
    attrs = AzureAttributes(
        azure_resource_id=azure.azure_resource_id,
        azure_location=azure.azure_location,
        adls_account_secondary_location=azure.adls_account_secondary_location,
        azure_tags=azure.azure_tags,
    )
    # Categorize relationships by save semantic (REPLACE, APPEND, REMOVE)
    rel_fields: list[str] = []
    replace_rels, append_rels, remove_rels = categorize_relationships(
        azure, rel_fields, AzureRelationshipAttributes
    )
    return AzureNested(
        guid=azure.guid,
