...
class MongoDB(Asset):
    ...
    # Override type_name with MongoDB-specific default
    ...
    def to_json(self, nested: bool = True, serde: Serde | None = None) -> str:
        ...
        if serde is None:
            serde = get_serde()
        if nested:
            return _mongo_db_to_nested_bytes(self, serde).decode("utf-8")
        else:
...
def _mongo_db_to_nested(mongo_db: MongoDB) -> MongoDBNested:
    ...
    attrs = MongoDBAttributes(
        no_sql_schema_definition=mongo_db.no_sql_schema_definition,
    )
    # Categorize relationships by save semantic (REPLACE, APPEND, REMOVE)
    rel_fields: list[str] = []
    replace_rels, append_rels, remove_rels = categorize_relationships(
        mongo_db, rel_fields, MongoDBRelationshipAttributes
    )
