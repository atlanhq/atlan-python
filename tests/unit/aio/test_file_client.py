...
@pytest.fixture()
def mock_session():
    with patch.object(AsyncAtlanClient, "_async_session") as mock_session:
        mock_response = Mock()
        ...
        mock_response.raise_for_status.return_value = None  # Should not raise

        # Create proper async iterator for aiter_bytes
        ...
        mock_session.stream.return_value = async_context_manager

        yield mock_session
    # Cleanup - remove download file if it exists (similar to sync version)
...
def mock_session_invalid():
    with patch.object(AsyncAtlanClient, "_async_session") as mock_session:
        ...
        # Create async iterator that raises the expected error
...
@pytest.mark.asyncio
async def test_async_file_client_upload_file_raises_invalid_request_error(
    mock_async_api_caller, file_path, expected_error
):
    client = AsyncFileClient(client=mock_async_api_caller)
...
@pytest.mark.asyncio
async def test_async_file_client_download_file_invalid_format_raises_invalid_request_error(
...
@pytest.mark.asyncio
async def test_async_file_client_get_presigned_url(
    mock_async_api_caller, s3_presigned_url
):
    mock_async_api_caller._call_api.return_value = {"url": s3_presigned_url}
    client = AsyncFileClient(mock_async_api_caller)
...
@patch.object(AsyncAtlanClient, "_call_api_internal", new_callable=AsyncMock)
@pytest.mark.asyncio
async def test_async_file_client_s3_upload_file(
    mock_call_api_internal, client, s3_presigned_url
):
    client = AsyncFileClient(client=client)
    await client.upload_file(presigned_url=s3_presigned_url, file_path=UPLOAD_FILE_PATH)
...
@patch.object(AsyncAtlanClient, "_call_api_internal", new_callable=AsyncMock)
@pytest.mark.asyncio
async def test_async_file_client_azure_blob_upload_file(
    mock_call_api_internal, client, blob_presigned_url
):
    client = AsyncFileClient(client=client)
    await client.upload_file(
        presigned_url=blob_presigned_url, file_path=UPLOAD_FILE_PATH
    )
...
@patch.object(AsyncAtlanClient, "_call_api_internal", new_callable=AsyncMock)
@pytest.mark.asyncio
async def test_async_file_client_gcs_upload_file(
    mock_call_api_internal, client, gcs_presigned_url
):
    client = AsyncFileClient(client=client)
    await client.upload_file(
        presigned_url=gcs_presigned_url, file_path=UPLOAD_FILE_PATH
    )
...
@pytest.mark.asyncio
async def test_async_file_client_download_file(client, s3_presigned_url, mock_session):
    # Make sure the download file doesn't exist before downloading
    assert not os.path.exists(DOWNLOAD_FILE_PATH)
    response = await client.files.download_file(
        presigned_url=s3_presigned_url, file_path=DOWNLOAD_FILE_PATH
    )
