...
def test_create_with_missing_parameters_raise_value_error(name: str, gcs_bucket_qualified_name: str, message: str):
    with pytest.raises(ValueError, match=message):
        GCSObject.create(name=name, gcs_bucket_qualified_name=gcs_bucket_qualified_name)


def test_create():
    sut = GCSObject.create(name=GCS_OBJECT_NAME, gcs_bucket_qualified_name=GCS_BUCKET_QUALIFIED_NAME)
...
def test_create_for_modification():
    sut = GCSObject.create_for_modification(qualified_name=GCS_OBJECT_QUALIFIED_NAME, name=GCS_OBJECT_NAME)
...
def test_trim_to_required():
    sut = GCSObject.create(name=GCS_OBJECT_NAME, gcs_bucket_qualified_name=GCS_BUCKET_QUALIFIED_NAME).trim_to_required()
