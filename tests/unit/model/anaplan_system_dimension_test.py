...
def test_create_with_missing_parameters_raise_value_error(name: str, connection_qualified_name: str, message: str):
    with pytest.raises(ValueError, match=message):
        AnaplanSystemDimension.create(name=name, connection_qualified_name=connection_qualified_name)
...
def test_create_for_modification_with_invalid_parameter_raises_value_error(
    ...
    with pytest.raises(ValueError, match=message):
        AnaplanSystemDimension.create_for_modification(qualified_name=qualified_name, name=name)
