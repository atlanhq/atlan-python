...
def test_creator_with_missing_parameters_raise_value_error(name: str, connection_qualified_name: str, message: str):
    with pytest.raises(ValueError, match=message):
        CustomEntity.creator(name=name, connection_qualified_name=connection_qualified_name)
...
def test_updater():
    sut = CustomEntity.updater(qualified_name=CUSTOM_ENTITY_QUALIFIED_NAME, name=CUSTOM_ENTITY_NAME)
...
def test_trim_to_required():
    sut = CustomEntity.updater(name=CUSTOM_ENTITY_NAME, qualified_name=CUSTOM_ENTITY_QUALIFIED_NAME).trim_to_required()
