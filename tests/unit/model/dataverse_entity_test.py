...
def test_creator_with_missing_parameters_raise_value_error(name: str, connection_qualified_name: str, message: str):
    with pytest.raises(ValueError, match=message):
        DataverseEntity.creator(name=name, connection_qualified_name=connection_qualified_name)
...
def test_updater():
    sut = DataverseEntity.updater(qualified_name=DATAVERSE_ENTITY_QUALIFIED_NAME, name=DATAVERSE_ENTITY_NAME)
