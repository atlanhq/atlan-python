...
def test_create_with_missing_parameters_raise_value_error(name: str, connection_qualified_name: str, message: str):
    with pytest.raises(ValueError, match=message):
        ADLSAccount.create(name=name, connection_qualified_name=connection_qualified_name)


def test_create():
    sut = ADLSAccount.create(name=ADLS_ACCOUNT_NAME, connection_qualified_name=ADLS_CONNECTION_QUALIFIED_NAME)
...
def test_create_for_modification():
    sut = ADLSAccount.create_for_modification(qualified_name=ADLS_QUALIFIED_NAME, name=ADLS_ACCOUNT_NAME)
