...
These tests verify APPEND, REMOVE, and REPLACE semantics work correctly
for managing Atlan tags on assets.
"""
...
def find_table_by_name(
    ...
    results = (
        ...
        .where(Table.QUALIFIED_NAME.wildcard(f"*/{connector_type}/*"))
...
def test_bulk_save_with_mixed_semantics(
    client: AtlanClient,
    table1: TableInfo,
    table2: TableInfo,
) -> None:
    """
    Test bulk save with APPEND and REPLACE semantics across different assets.
    - Table1: APPEND Issue and Confidential
    - Table2: REPLACE with Issue only
    Expected: Both tables updated correctly based on their semantics.
    """
    # First add Confidential to Table2 to test REPLACE
    table2_setup = Table.updater(qualified_name=table2.qualified_name, name=table2.name)
    table2_setup.atlan_tags = [
        AtlanTag.of(
            atlan_tag_name=AtlanTagName(TAG_CONFIDENTIAL),
            semantic=SaveSemantic.APPEND,
        ),
    ]
    client.asset.save(entity=table2_setup)
    time.sleep(3)

    # Verify setup
    tags_table2_setup = get_current_tags(client, table2.qualified_name)
    assert TAG_CONFIDENTIAL in tags_table2_setup, (
        "Setup failed: Confidential not on Table2"
    )

    # Bulk save with different semantics
    ...
    response = client.asset.save(entity=[t1, t2])
    time.sleep(3)

    # Verify response
    assert response is not None

    # Check Table1 - should have both tags (APPEND)
    tags1 = get_current_tags(client, table1.qualified_name)
    assert TAG_ISSUE in tags1, f"Table1: Expected '{TAG_ISSUE}' in tags"
    assert TAG_CONFIDENTIAL in tags1, f"Table1: Expected '{TAG_CONFIDENTIAL}' in tags"

    # Check Table2 - should only have Issue (REPLACE removed Confidential)
    tags2 = get_current_tags(client, table2.qualified_name)
    assert TAG_ISSUE in tags2, f"Table2: Expected '{TAG_ISSUE}' in tags"
    assert TAG_CONFIDENTIAL not in tags2, (
        f"Table2: REPLACE should have removed '{TAG_CONFIDENTIAL}'"
    )

    LOGGER.info(f"Bulk save test passed. Table1 tags: {tags1}, Table2 tags: {tags2}")


@pytest.mark.order(after="test_bulk_save_with_mixed_semantics")
