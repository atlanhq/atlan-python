...
def connection(client: AtlanClient):
    ...
    response = client.open_lineage.create_connection(name=MODULE_NAME, admin_roles=[admin_role_guid])
    result = response.assets_created(asset_type=Connection)[0]
    yield client.asset.get_by_guid(result.guid, asset_type=Connection, ignore_relationships=False)
...
def test_open_lineage_integration(connection: Connection, client: AtlanClient):
    ...
    job = OpenLineageJob.creator(connection_name=MODULE_NAME, job_name="dag_123", producer=producer)
    ...
    input_qns = {input.get("uniqueAttributes", {}).get("qualifiedName") for input in inputs}
    ...
    outputs_qns = {output.get("uniqueAttributes", {}).get("qualifiedName") for output in outputs}
    ...
    assert process.get("uniqueAttributes", {}).get("qualifiedName") == f"{connection.qualified_name}/dag_123/process"
