...
class AsyncWorkflowClient:
    ...
    async def find_runs_by_status_and_time_range(
        ...
        run_lookup_results = await self._find_runs(
    ...
    async def _find_runs(
        ...
        raw_json = await self._client._call_api(endpoint, request_obj=request_obj)
        sync_response = WorkflowFindRuns.process_response(raw_json)
    ...
    @validate_arguments
    async def update(self, workflow: Workflow) -> WorkflowResponse:
        """
    ...
    async def monitor(
        ...
        while status not in {
            ...
            if run_details := await self._get_run_details(name):
    ...
    async def _get_run_details(self, name: str) -> Optional[WorkflowSearchResult]:
        return await self._find_latest_run(workflow_name=name)
    ...
    async def get_runs(
        ...
        response = await self._find_runs(query, from_=from_, size=size)
        return response
    ...
    async def find_schedule_query_between(
        ...
        raw_json = await self._client._call_api(endpoint, query_params=request_obj)
