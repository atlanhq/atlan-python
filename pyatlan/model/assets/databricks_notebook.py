...
class DatabricksNotebook(Notebook):
    """Description"""

    type_name: str = Field(default="DatabricksNotebook", allow_mutation=False)

    @validator("type_name")
    def validate_type_name(cls, v):
        if v != "DatabricksNotebook":
            raise ValueError("must be DatabricksNotebook")
        return v

    def __setattr__(self, name, value):
        if name in DatabricksNotebook._convenience_properties:
            return object.__setattr__(self, name, value)
        super().__setattr__(name, value)

    DATABRICKS_NOTEBOOK_PATH: ClassVar[KeywordField] = KeywordField(
        "databricksNotebookPath", "databricksNotebookPath"
    )
    """
    Path of the notebook.
    """
    DATABRICKS_NOTEBOOK_WORKSPACE_ID: ClassVar[KeywordField] = KeywordField(
    ...
    _convenience_properties: ClassVar[List[str]] = [
        ...
        "databricks_notebook_workspace_id",
        "query_count",
        "query_user_count",
        "query_user_map",
    ...
    @property
    def databricks_notebook_path(self) -> Optional[str]:
        return (
            None
            if self.attributes is None
            else self.attributes.databricks_notebook_path
        )

    @databricks_notebook_path.setter
    def databricks_notebook_path(self, databricks_notebook_path: Optional[str]):
        if self.attributes is None:
            self.attributes = self.Attributes()
        self.attributes.databricks_notebook_path = databricks_notebook_path

    @property
    def databricks_notebook_workspace_id(self) -> Optional[str]:
        return (
            None
            if self.attributes is None
            else self.attributes.databricks_notebook_workspace_id
        )

    @databricks_notebook_workspace_id.setter
    def databricks_notebook_workspace_id(
        self, databricks_notebook_workspace_id: Optional[str]
    ):
        if self.attributes is None:
            self.attributes = self.Attributes()
        self.attributes.databricks_notebook_workspace_id = (
            databricks_notebook_workspace_id
        )
    ...
    class Attributes(Notebook.Attributes):
        databricks_notebook_path: Optional[str] = Field(default=None, description="")
        databricks_notebook_workspace_id: Optional[str] = Field(
    ...
    attributes: DatabricksNotebook.Attributes = Field(
        default_factory=lambda: DatabricksNotebook.Attributes(),
        description=(
...
DatabricksNotebook.Attributes.update_forward_refs()
