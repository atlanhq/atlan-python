...
class DremioColumn(Dremio):
    ...
    type_name: str = Field(default="DremioColumn", allow_mutation=False)

    @validator("type_name")
    def validate_type_name(cls, v):
        if v != "DremioColumn":
            raise ValueError("must be DremioColumn")
        return v

    def __setattr__(self, name, value):
        if name in DremioColumn._convenience_properties:
    ...
    attributes: DremioColumn.Attributes = Field(
        default_factory=lambda: DremioColumn.Attributes(),
        description=(
