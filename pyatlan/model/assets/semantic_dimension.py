...
class SemanticDimension(Semantic):
    """Description"""

    type_name: str = Field(default="SemanticDimension", allow_mutation=False)

    @validator("type_name")
    def validate_type_name(cls, v):
        if v != "SemanticDimension":
            raise ValueError("must be SemanticDimension")
        return v

    def __setattr__(self, name, value):
        if name in SemanticDimension._convenience_properties:
    ...
    attributes: SemanticDimension.Attributes = Field(
        default_factory=lambda: SemanticDimension.Attributes(),
        description=(
...
SemanticDimension.Attributes.update_forward_refs()
