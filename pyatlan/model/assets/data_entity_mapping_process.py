...
class DataEntityMappingProcess(Process):
    """Description"""

    type_name: str = Field(default="DataEntityMappingProcess", allow_mutation=False)

    @validator("type_name")
    def validate_type_name(cls, v):
        if v != "DataEntityMappingProcess":
            raise ValueError("must be DataEntityMappingProcess")
        return v

    def __setattr__(self, name, value):
        if name in DataEntityMappingProcess._convenience_properties:
    ...
    attributes: DataEntityMappingProcess.Attributes = Field(
        default_factory=lambda: DataEntityMappingProcess.Attributes(),
        description=(
