...
class Asset(Referenceable):
    ...
    def trim_to_reference(self: SelfAsset) -> SelfAsset:
        if bool(self.guid and self.guid.strip()):
            return self.ref_by_guid(self.guid)
        if bool(self.qualified_name and self.qualified_name.strip()):
            return self.ref_by_qualified_name(str(self.qualified_name))
        if bool(
            self.unique_attributes
            and self.unique_attributes.get("qualified_name")
            and str(self.unique_attributes.get("qualified_name")).strip()
        ):
            return self.ref_by_qualified_name(
                str(cast(dict[str, Any], self.unique_attributes).get("qualified_name"))
            )
