...
class AzureServiceBusSchema(AzureServiceBus):
    """Description"""

    type_name: str = Field(default="AzureServiceBusSchema", allow_mutation=False)

    @validator("type_name")
    def validate_type_name(cls, v):
        if v != "AzureServiceBusSchema":
            raise ValueError("must be AzureServiceBusSchema")
        return v

    def __setattr__(self, name, value):
        if name in AzureServiceBusSchema._convenience_properties:
    ...
    attributes: AzureServiceBusSchema.Attributes = Field(
        default_factory=lambda: AzureServiceBusSchema.Attributes(),
        description=(
...
AzureServiceBusSchema.Attributes.update_forward_refs()
