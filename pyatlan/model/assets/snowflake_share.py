...
from pyatlan.model.enums import SnowflakeShareKind
...
class SnowflakeShare(Snowflake):
    """Description"""

    type_name: str = Field(default="SnowflakeShare", allow_mutation=False)

    @validator("type_name")
    def validate_type_name(cls, v):
        if v != "SnowflakeShare":
            raise ValueError("must be SnowflakeShare")
        return v

    def __setattr__(self, name, value):
        if name in SnowflakeShare._convenience_properties:
            return object.__setattr__(self, name, value)
        super().__setattr__(name, value)

    SNOWFLAKE_SHARE_KIND: ClassVar[KeywordField] = KeywordField(
        "snowflakeShareKind", "snowflakeShareKind"
    )
    """
    Direction of the share (inbound or outbound).
    """
    SNOWFLAKE_SHARE_OWNER_ACCOUNT: ClassVar[KeywordField] = KeywordField(
        "snowflakeShareOwnerAccount", "snowflakeShareOwnerAccount"
    )
    """
    Account that owns the share. Drives the share qualified name.
    """
    SNOWFLAKE_SHARE_TARGET_ACCOUNTS: ClassVar[KeywordField] = KeywordField(
        "snowflakeShareTargetAccounts", "snowflakeShareTargetAccounts"
    )
    """
    Consumer accounts targeted by the share.
    """
    SNOWFLAKE_SHARE_LISTING_GLOBAL_NAME: ClassVar[KeywordField] = KeywordField(
        "snowflakeShareListingGlobalName", "snowflakeShareListingGlobalName"
    )
    """
    Global name of the listing this share is bound to.
    """
    SNOWFLAKE_SHARE_SECURE_OBJECT: ClassVar[BooleanField] = BooleanField(
    ...
    SNOWFLAKE_LISTING: ClassVar[RelationField] = RelationField("snowflakeListing")
    """
    TBC
    """

    _convenience_properties: ClassVar[List[str]] = [
        "snowflake_share_kind",
        "snowflake_share_owner_account",
        "snowflake_share_target_accounts",
        "snowflake_share_listing_global_name",
        "snowflake_share_secure_object",
        "query_count",
        "query_user_count",
        "query_user_map",
    ...
    @property
    def snowflake_share_kind(self) -> Optional[SnowflakeShareKind]:
        return None if self.attributes is None else self.attributes.snowflake_share_kind

    @snowflake_share_kind.setter
    def snowflake_share_kind(self, snowflake_share_kind: Optional[SnowflakeShareKind]):
        if self.attributes is None:
            self.attributes = self.Attributes()
        self.attributes.snowflake_share_kind = snowflake_share_kind

    @property
    def snowflake_share_owner_account(self) -> Optional[str]:
        return (
            None
            if self.attributes is None
            else self.attributes.snowflake_share_owner_account
        )

    @snowflake_share_owner_account.setter
    def snowflake_share_owner_account(
        self, snowflake_share_owner_account: Optional[str]
    ):
        if self.attributes is None:
            self.attributes = self.Attributes()
        self.attributes.snowflake_share_owner_account = snowflake_share_owner_account

    @property
    def snowflake_share_target_accounts(self) -> Optional[Set[str]]:
        return (
            None
            if self.attributes is None
            else self.attributes.snowflake_share_target_accounts
        )

    @snowflake_share_target_accounts.setter
    def snowflake_share_target_accounts(
        self, snowflake_share_target_accounts: Optional[Set[str]]
    ):
        if self.attributes is None:
            self.attributes = self.Attributes()
        self.attributes.snowflake_share_target_accounts = (
            snowflake_share_target_accounts
        )

    @property
    def snowflake_share_listing_global_name(self) -> Optional[str]:
        return (
            None
            if self.attributes is None
            else self.attributes.snowflake_share_listing_global_name
        )

    @snowflake_share_listing_global_name.setter
    def snowflake_share_listing_global_name(
        self, snowflake_share_listing_global_name: Optional[str]
    ):
        if self.attributes is None:
            self.attributes = self.Attributes()
        self.attributes.snowflake_share_listing_global_name = (
            snowflake_share_listing_global_name
        )

    @property
    def snowflake_share_secure_object(self) -> Optional[bool]:
        return (
            None
            if self.attributes is None
            else self.attributes.snowflake_share_secure_object
        )

    @snowflake_share_secure_object.setter
    def snowflake_share_secure_object(
        self, snowflake_share_secure_object: Optional[bool]
    ):
        if self.attributes is None:
            self.attributes = self.Attributes()
        self.attributes.snowflake_share_secure_object = snowflake_share_secure_object
    ...
    @property
    def snowflake_listing(self) -> Optional[SnowflakeListing]:
        return None if self.attributes is None else self.attributes.snowflake_listing

    @snowflake_listing.setter
    def snowflake_listing(self, snowflake_listing: Optional[SnowflakeListing]):
        if self.attributes is None:
            self.attributes = self.Attributes()
        self.attributes.snowflake_listing = snowflake_listing

    class Attributes(Snowflake.Attributes):
        snowflake_share_kind: Optional[SnowflakeShareKind] = Field(
            default=None, description=""
        )
        snowflake_share_owner_account: Optional[str] = Field(
            default=None, description=""
        )
        snowflake_share_target_accounts: Optional[Set[str]] = Field(
            default=None, description=""
        )
        snowflake_share_listing_global_name: Optional[str] = Field(
            default=None, description=""
        )
        snowflake_share_secure_object: Optional[bool] = Field(
            default=None, description=""
        )
        ...
        snowflake_listing: Optional[SnowflakeListing] = Field(
            default=None, description=""
        )  # relationship

    attributes: SnowflakeShare.Attributes = Field(
        default_factory=lambda: SnowflakeShare.Attributes(),
        description=(
...
from .snowflake_listing import SnowflakeListing  # noqa: E402, F401

SnowflakeShare.Attributes.update_forward_refs()
