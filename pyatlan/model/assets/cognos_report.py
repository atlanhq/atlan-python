...
class CognosReport(Cognos):
    """Description"""

    type_name: str = Field(default="CognosReport", allow_mutation=False)

    @validator("type_name")
    def validate_type_name(cls, v):
        if v != "CognosReport":
            raise ValueError("must be CognosReport")
        return v

    def __setattr__(self, name, value):
        if name in CognosReport._convenience_properties:
    ...
    attributes: CognosReport.Attributes = Field(
        default_factory=lambda: CognosReport.Attributes(),
        description=(
