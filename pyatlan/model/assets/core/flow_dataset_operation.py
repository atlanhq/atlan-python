...
class FlowDatasetOperation(Process):
    """Description"""

    type_name: str = Field(default="FlowDatasetOperation", allow_mutation=False)

    @validator("type_name")
    def validate_type_name(cls, v):
        if v != "FlowDatasetOperation":
            raise ValueError("must be FlowDatasetOperation")
        return v

    def __setattr__(self, name, value):
        if name in FlowDatasetOperation._convenience_properties:
    ...
    @flow_reusable_unit.setter
    def flow_reusable_unit(self, flow_reusable_unit: Optional[FlowReusableUnit]):
        if self.attributes is None:
            self.attributes = self.Attributes()
        self.attributes.flow_reusable_unit = flow_reusable_unit
    ...
    attributes: FlowDatasetOperation.Attributes = Field(
        default_factory=lambda: FlowDatasetOperation.Attributes(),
        description=(
