...
class StarburstDatasetColumn(Column):
    """Description"""

    type_name: str = Field(default="StarburstDatasetColumn", allow_mutation=False)

    @validator("type_name")
    def validate_type_name(cls, v):
        if v != "StarburstDatasetColumn":
            raise ValueError("must be StarburstDatasetColumn")
        return v

    def __setattr__(self, name, value):
        if name in StarburstDatasetColumn._convenience_properties:
            return object.__setattr__(self, name, value)
        super().__setattr__(name, value)

    STARBURST_SQL_COLUMN_QUALIFIED_NAME: ClassVar[KeywordField] = KeywordField(
    ...
    STARBURST_DATASET: ClassVar[RelationField] = RelationField("starburstDataset")
    """
    TBC
    """

    _convenience_properties: ClassVar[List[str]] = [
        "starburst_sql_column_qualified_name",
        "data_type",
        "sub_data_type",
        "column_compression",
        "column_encoding",
    ...
    @property
    def starburst_sql_column_qualified_name(self) -> Optional[str]:
        return (
            None
            if self.attributes is None
            else self.attributes.starburst_sql_column_qualified_name
        )

    @starburst_sql_column_qualified_name.setter
    def starburst_sql_column_qualified_name(
        self, starburst_sql_column_qualified_name: Optional[str]
    ):
        if self.attributes is None:
            self.attributes = self.Attributes()
        self.attributes.starburst_sql_column_qualified_name = (
            starburst_sql_column_qualified_name
        )
    ...
    @property
    def starburst_dataset(self) -> Optional[StarburstDataset]:
        return None if self.attributes is None else self.attributes.starburst_dataset

    @starburst_dataset.setter
    def starburst_dataset(self, starburst_dataset: Optional[StarburstDataset]):
        if self.attributes is None:
            self.attributes = self.Attributes()
        self.attributes.starburst_dataset = starburst_dataset

    class Attributes(Column.Attributes):
        starburst_sql_column_qualified_name: Optional[str] = Field(
        ...
        starburst_dataset: Optional[StarburstDataset] = Field(
            default=None, description=""
        )  # relationship

    attributes: StarburstDatasetColumn.Attributes = Field(
        default_factory=lambda: StarburstDatasetColumn.Attributes(),
        description=(
...
from .starburst_dataset import StarburstDataset  # noqa: E402, F401
