...
class DocumentDBDatabase(DocumentDB):
    ...
    def validate_type_name(cls, v):
        if v != "DocumentDBDatabase":
            raise ValueError("must be DocumentDBDatabase")
        return v

    def __setattr__(self, name, value):
        if name in DocumentDBDatabase._convenience_properties:
            return object.__setattr__(self, name, value)
        super().__setattr__(name, value)

    DOCUMENT_DB_DATABASE_COLLECTION_COUNT: ClassVar[NumericField] = NumericField(
    ...
    @property
    def document_d_b_database_collection_count(self) -> Optional[int]:
        return (
            None
            if self.attributes is None
            else self.attributes.document_d_b_database_collection_count
        )

    @document_d_b_database_collection_count.setter
    def document_d_b_database_collection_count(
        self, document_d_b_database_collection_count: Optional[int]
    ):
        if self.attributes is None:
            self.attributes = self.Attributes()
        self.attributes.document_d_b_database_collection_count = (
            document_d_b_database_collection_count
        )
    ...
    @property
    def document_d_b_collections(self) -> Optional[List[DocumentDBCollection]]:
        return (
            None
            if self.attributes is None
            else self.attributes.document_d_b_collections
        )

    @document_d_b_collections.setter
    def document_d_b_collections(
        self, document_d_b_collections: Optional[List[DocumentDBCollection]]
    ):
        if self.attributes is None:
            self.attributes = self.Attributes()
    ...
    class Attributes(DocumentDB.Attributes):
        document_d_b_database_collection_count: Optional[int] = Field(
