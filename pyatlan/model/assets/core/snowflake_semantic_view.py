...
class SnowflakeSemanticView(Snowflake):
    """Description"""

    type_name: str = Field(default="SnowflakeSemanticView", allow_mutation=False)

    @validator("type_name")
    def validate_type_name(cls, v):
        if v != "SnowflakeSemanticView":
            raise ValueError("must be SnowflakeSemanticView")
        return v

    def __setattr__(self, name, value):
        if name in SnowflakeSemanticView._convenience_properties:
    ...
    SNOWFLAKE_SEMANTIC_VIEW_SCHEMA: ClassVar[RelationField] = RelationField(
    ...
    @property
    def snowflake_definition(self) -> Optional[str]:
        return None if self.attributes is None else self.attributes.snowflake_definition

    @snowflake_definition.setter
    def snowflake_definition(self, snowflake_definition: Optional[str]):
        if self.attributes is None:
            self.attributes = self.Attributes()
        self.attributes.snowflake_definition = snowflake_definition
    ...
    @property
    def snowflake_semantic_view_schema(self) -> Optional[Schema]:
        return (
            None
            if self.attributes is None
            else self.attributes.snowflake_semantic_view_schema
        )

    @snowflake_semantic_view_schema.setter
    def snowflake_semantic_view_schema(
        self, snowflake_semantic_view_schema: Optional[Schema]
    ):
        if self.attributes is None:
            self.attributes = self.Attributes()
        self.attributes.snowflake_semantic_view_schema = snowflake_semantic_view_schema
    ...
    class Attributes(Snowflake.Attributes):
        snowflake_definition: Optional[str] = Field(default=None, description="")
    ...
    attributes: SnowflakeSemanticView.Attributes = Field(
        default_factory=lambda: SnowflakeSemanticView.Attributes(),
        description=(
