...
class StarburstDataset(Table):
    """Description"""

    type_name: str = Field(default="StarburstDataset", allow_mutation=False)

    @validator("type_name")
    def validate_type_name(cls, v):
        if v != "StarburstDataset":
            raise ValueError("must be StarburstDataset")
        return v

    def __setattr__(self, name, value):
        if name in StarburstDataset._convenience_properties:
            return object.__setattr__(self, name, value)
        super().__setattr__(name, value)

    STARBURST_IS_MATERIALIZED: ClassVar[BooleanField] = BooleanField(
        "starburstIsMaterialized", "starburstIsMaterialized"
    )
    """
    Whether this dataset is a materialized view.
    """
    STARBURST_SQL_QUALIFIED_NAME: ClassVar[KeywordField] = KeywordField(
    ...
    """  # noqa: E501
    STARBURST_VIEW_DEFINITION: ClassVar[KeywordField] = KeywordField(
    ...
    STARBURST_DATASET_COLUMNS: ClassVar[RelationField] = RelationField(
        "starburstDatasetColumns"
    )
    """
    TBC
    """
    STARBURST_DATA_PRODUCT: ClassVar[RelationField] = RelationField(
        "starburstDataProduct"
    )
    """
    TBC
    """
    ...
    _convenience_properties: ClassVar[List[str]] = [
        "starburst_is_materialized",
        "starburst_sql_qualified_name",
        "starburst_view_definition",
        "column_count",
        "row_count",
        "size_bytes",
        "table_object_count",
    ...
    @property
    def starburst_is_materialized(self) -> Optional[bool]:
        return (
            None
            if self.attributes is None
            else self.attributes.starburst_is_materialized
        )

    @starburst_is_materialized.setter
    def starburst_is_materialized(self, starburst_is_materialized: Optional[bool]):
        if self.attributes is None:
            self.attributes = self.Attributes()
        self.attributes.starburst_is_materialized = starburst_is_materialized

    @property
    def starburst_sql_qualified_name(self) -> Optional[str]:
        return (
            None
            if self.attributes is None
            else self.attributes.starburst_sql_qualified_name
        )

    @starburst_sql_qualified_name.setter
    def starburst_sql_qualified_name(self, starburst_sql_qualified_name: Optional[str]):
        if self.attributes is None:
            self.attributes = self.Attributes()
        self.attributes.starburst_sql_qualified_name = starburst_sql_qualified_name

    @property
    def starburst_view_definition(self) -> Optional[str]:
        return (
            None
            if self.attributes is None
            else self.attributes.starburst_view_definition
        )

    @starburst_view_definition.setter
    def starburst_view_definition(self, starburst_view_definition: Optional[str]):
        if self.attributes is None:
            self.attributes = self.Attributes()
        self.attributes.starburst_view_definition = starburst_view_definition
    ...
    @property
    def starburst_dataset_columns(self) -> Optional[List[StarburstDatasetColumn]]:
        return (
            None
            if self.attributes is None
            else self.attributes.starburst_dataset_columns
        )

    @starburst_dataset_columns.setter
    def starburst_dataset_columns(
        self, starburst_dataset_columns: Optional[List[StarburstDatasetColumn]]
    ):
        if self.attributes is None:
            self.attributes = self.Attributes()
        self.attributes.starburst_dataset_columns = starburst_dataset_columns

    @property
    def starburst_data_product(self) -> Optional[DataProduct]:
        return (
            None if self.attributes is None else self.attributes.starburst_data_product
        )

    @starburst_data_product.setter
    def starburst_data_product(self, starburst_data_product: Optional[DataProduct]):
        if self.attributes is None:
            self.attributes = self.Attributes()
        self.attributes.starburst_data_product = starburst_data_product

    class Attributes(Table.Attributes):
        starburst_is_materialized: Optional[bool] = Field(default=None, description="")
        starburst_sql_qualified_name: Optional[str] = Field(
            default=None, description=""
        )
        starburst_view_definition: Optional[str] = Field(default=None, description="")
        ...
        starburst_dataset_columns: Optional[List[StarburstDatasetColumn]] = Field(
            default=None, description=""
        )  # relationship
        starburst_data_product: Optional[DataProduct] = Field(
            default=None, description=""
        )  # relationship

    attributes: StarburstDataset.Attributes = Field(
        default_factory=lambda: StarburstDataset.Attributes(),
        description=(
...
from .starburst_dataset_column import StarburstDatasetColumn  # noqa: E402, F401
