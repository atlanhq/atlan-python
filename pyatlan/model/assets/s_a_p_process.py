...
class SAPProcess(SAP):
    ...
    type_name: str = Field(default="SAPProcess", allow_mutation=False)

    @validator("type_name")
    def validate_type_name(cls, v):
        if v != "SAPProcess":
            raise ValueError("must be SAPProcess")
        return v

    def __setattr__(self, name, value):
        if name in SAPProcess._convenience_properties:
    ...
    attributes: SAPProcess.Attributes = Field(
        default_factory=lambda: SAPProcess.Attributes(),
        description=(
