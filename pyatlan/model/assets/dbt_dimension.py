...
class DbtDimension(SemanticDimension):
    """Description"""

    type_name: str = Field(default="DbtDimension", allow_mutation=False)

    @validator("type_name")
    def validate_type_name(cls, v):
        if v != "DbtDimension":
            raise ValueError("must be DbtDimension")
        return v

    def __setattr__(self, name, value):
        if name in DbtDimension._convenience_properties:
            return object.__setattr__(self, name, value)
        super().__setattr__(name, value)

    DBT_SEMANTIC_MODEL_QUALIFIED_NAME: ClassVar[KeywordField] = KeywordField(
    ...
    """
    DBT_SEMANTIC_FIELD_TIME_GRANULARITY: ClassVar[KeywordField] = KeywordField(
    ...
    _convenience_properties: ClassVar[List[str]] = [
        ...
        "dbt_semantic_field_time_granularity",
        "semantic_expression",
        "semantic_type",
        "dbt_alias",
        "dbt_meta",
    ...
    @property
    def dbt_semantic_model_qualified_name(self) -> Optional[str]:
        return (
            None
            if self.attributes is None
            else self.attributes.dbt_semantic_model_qualified_name
        )

    @dbt_semantic_model_qualified_name.setter
    def dbt_semantic_model_qualified_name(
        self, dbt_semantic_model_qualified_name: Optional[str]
    ):
        if self.attributes is None:
            self.attributes = self.Attributes()
        self.attributes.dbt_semantic_model_qualified_name = (
            dbt_semantic_model_qualified_name
        )

    @property
    def dbt_semantic_field_time_granularity(self) -> Optional[str]:
        return (
            None
            if self.attributes is None
            else self.attributes.dbt_semantic_field_time_granularity
        )

    @dbt_semantic_field_time_granularity.setter
    def dbt_semantic_field_time_granularity(
        self, dbt_semantic_field_time_granularity: Optional[str]
    ):
        if self.attributes is None:
            self.attributes = self.Attributes()
        self.attributes.dbt_semantic_field_time_granularity = (
            dbt_semantic_field_time_granularity
        )
    ...
    class Attributes(SemanticDimension.Attributes):
        ...
        dbt_semantic_field_time_granularity: Optional[str] = Field(
            default=None, description=""
        )
    ...
    attributes: DbtDimension.Attributes = Field(
        default_factory=lambda: DbtDimension.Attributes(),
        description=(
...
DbtDimension.Attributes.update_forward_refs()
