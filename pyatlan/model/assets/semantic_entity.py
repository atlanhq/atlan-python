...
class SemanticEntity(Semantic):
    """Description"""

    type_name: str = Field(default="SemanticEntity", allow_mutation=False)

    @validator("type_name")
    def validate_type_name(cls, v):
        if v != "SemanticEntity":
            raise ValueError("must be SemanticEntity")
        return v

    def __setattr__(self, name, value):
        if name in SemanticEntity._convenience_properties:
    ...
    attributes: SemanticEntity.Attributes = Field(
        default_factory=lambda: SemanticEntity.Attributes(),
        description=(
...
SemanticEntity.Attributes.update_forward_refs()
