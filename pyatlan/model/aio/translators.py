...
class AsyncBaseTranslator(ABC):
    """
    Abstract base class for async response translators that determine
    ...
    async def translate(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """
        Performs async transformation on the provided dictionary.
        """
        pass
...
class AsyncAtlanTagTranslator(AsyncBaseTranslator):
    """
    Async translator responsible for converting
    ...
    def __init__(self, client: AsyncAtlanClient):
        """
        Initialize the translator with the async Atlan client.
        """
        self.client = client
    ...
    async def translate(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """
        Async conversion of hashed tag IDs in classification fields into human-readable tag names.
        ...
        for key in self._CLASSIFICATION_NAMES:
            if key in raw_json:
                ...
                for tag_id in raw_json[key]:
                    ...
                    tag_names.append(tag_name or DELETED_)
        ...
        for key in self._CLASSIFICATION_KEYS:
            if key in raw_json:
                for classification in raw_json[key]:
                    ...
                    if tag_id:
                        tag_name = await self.client.atlan_tag_cache.get_name_for_id(
                            tag_id
                        )
