...
"""Models for the App workflow APIs.

This surface uses **snake_case** wire keys (``app_id``, ``run_id``,
...
class AppObject(AtlanObject):
    """Base for App models: snake_case wire keys (no camelCase aliasing)."""

    class Config(AtlanObject.Config):
        # keep field names as-is (snake_case on the wire)
        alias_generator = None  # type: ignore[assignment]
