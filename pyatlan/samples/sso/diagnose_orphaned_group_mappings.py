...
class SSOGroupMappingDiagnostic:
    ...
    def diagnose_orphaned_mappings(
        ...
        for mapping in mappings:
            ...
            mapping_group_id = (
                mapping.name.split("--")[0] if "--" in mapping.name else None
            )
            ...
            # Case 3: Group ID in mapping doesn't match current group with that name
                ...
                mapping_group_id
                and group_name_to_id.get(mapped_group_name) != mapping_group_id
            ):
                is_orphaned = True
    ...
    def cleanup_orphaned_mappings(
        ...
        for mapping in orphaned:
            if not mapping.id:
                logger.warning("Skipping mapping with no ID")
                continue
    ...
    try:
        if args.mode == "list":
            ...
            if args.non_interactive:
                ...
                logger.warning(
                    "⚠️  All orphaned mappings will be deleted automatically!"
                )
                response = (
            ...
            if deleted > 0:
                ...
                logger.info("\n💡 NEXT STEPS:")
                logger.info(
                    "1. In Okta, try unlinking and re-linking the affected group"
                )
                logger.info("2. Use 'Link Group' (not 'Create') when re-linking")
                logger.info(
                    "3. The group push should now work without the stale externalId error"
                )
