...
class AsyncRoleCache:
    """
    Async lazily-loaded cache for translating Atlan-internal roles into their various IDs.
    """
    ...
    async def validate_idstrs(self, idstrs: Iterable[str]):
        """
        ...
        for role_id in idstrs:
            if not await self.get_name_for_id(role_id):
                raise ValueError(f"Provided role ID {role_id} was not found in Atlan.")

    async def refresh_cache(self) -> None:
        """
        Refreshes the cache of roles by requesting the full set of roles from Atlan.
        """
        await self._refresh_cache()

    async def _refresh_cache(self) -> None:
        async with self.lock:
            response = await self.client.role.get(
    ...
    async def is_api_token_user(self) -> bool:
        """
        ...
        :returns: True if the user is an API token user or admin, False otherwise
        """
